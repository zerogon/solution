...
const DEPARTMENT_NAME_MAP: Record<string, string> = {
  ...
  '현대실용음악과': '현대실용음악학과',
  '실용음악전공': '실용음악전공',
  'PostModern음악과': 'PostModern음악학과',
  '공연예술학부 실용음악': '공연예술학부 실용음악전공',
  '공연예술학부(실용음악)': '공연예술학부(실용음악전공)',
  '응용음악과': '실용음악학과',
};
...
const DEPARTMENT_CATEGORY_RULES: Array<{
  ...
  { keywords: ['음악', '예술', '공연', '작곡', '보컬', '싱어'], category: '예체능계열' },
];
...
/**
 * 국립/공립 대학교 목록
 * 대학명으로 설립 유형을 추론할 때 사용
 */
const NATIONAL_UNIVERSITIES = new Set([
  // 거점 국립대
  '서울대학교', '부산대학교', '경북대학교', '전남대학교',
  '전북대학교', '충남대학교', '충북대학교', '강원대학교',
  '제주대학교', '경상국립대학교',
  // 기타 국립대
  '창원대학교', '군산대학교', '목포대학교', '순천대학교',
  '안동대학교', '한국교통대학교', '인천대학교',
  '서울과학기술대학교', '한국해양대학교', '부경대학교',
  '한밭대학교', '금오공과대학교', '한경국립대학교',
  '공주대학교', '한국체육대학교', '한국방송통신대학교',
  '서울시립대학교',
  // 과학기술원
  '한국과학기술원', '포항공과대학교', '울산과학기술원',
  '광주과학기술원', '대구경북과학기술원',
  // 교육대학
  '서울교육대학교', '경인교육대학교', '공주교육대학교',
  '광주교육대학교', '대구교육대학교', '부산교육대학교',
  '전주교육대학교', '진주교육대학교', '청주교육대학교',
  '춘천교육대학교',
  // 예술대학
  '한국예술종합학교',
]);
...
/**
 * 대학명으로 설립 유형 추론
 * 국립대 목록에 있으면 '국립', 없으면 '사립' 반환
 */
export function inferUniversityType(universityName: string): string {
  const normalized = normalizeUniversityName(universityName);
  if (NATIONAL_UNIVERSITIES.has(normalized)) return '국립';
  return '사립';
}
