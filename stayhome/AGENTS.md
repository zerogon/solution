...
npx tsx scripts/debug-holidays.ts calc 2026-10-05 # 2단계를 한 줄씩
```

**규칙은 영업일 카운트가 아니고, 기준일은 보정하지 않는다**(2026-09-09 교체).
① 기준일에서 **달력 10일을 뺀다**(= `-10`일. 기준일이 주말·공휴일이어도 그대로) →
② 그 날이 주말/공휴일이면 직전 영업일로 당긴다. 옛 규칙(09-02)에 있던 "기준일이 휴일이면
먼저 당긴다"와 "기준일을 1일째로 포함(−9)"이 둘 다 사라졌다.

`calc`가 표를 찍는 이유는 오크밸리 `probe`·한화 `cal`과 같다: 답이 틀렸을 때 "틀렸다"가
아니라 **어디서 갈렸는지**가 보여야 한다. 기준선은 `2026-10-05(월, 쉬는 날 개천절) →
2026-09-23(수)` — 기준일이 공휴일인데 보정 없이 −10일 = 9/25 추석 → 9/24 추석 연휴 → 9/23.
짝은 `2026-10-19(월) → 2026-10-08(목)`(10/9 한글날 → 10/8).
...
- **`golden`의 판별 케이스 둘이 새 규칙의 두 축을 각각 혼자 지킨다.** 운영자 예시
  둘(10/5·10/19)은 옛 규칙과 새 규칙이 **같은 답**을 내서 그 둘만으로는 교체가 검증되지
  않는다. `2026-08-17(쉬는 날 광복절) → 2026-08-07`은 "기준일을 보정하지 않는다"를
  (옛 규칙이면 8/5), `2026-10-16 → 2026-10-06`은 "기준일 비포함 −n"을(−(n−1)이면 10/7) 고정한다.
...
~~**판정(2026-08-26): 한화는 여기서 멈춘다.**~~ (2026-09-24에 풀렸다 — 아래 "### 요금표는 JSON이었다") 불가능해서가 아니라 배선 하나가 아니라서다 —
107종 객실유형 지도 + 16지점 `bp_cd` 지도 + 연간 시즌 어휘 + 회원 등급, 넷이 다 필요하다.
오크밸리에서 같은 일을 9종·2지점·공개 달력으로 한 것과 규모가 다르다.

### 요금표는 JSON이었다 — 조인 키가 재고와 같다 (2026-09-24 배선)

위 `axes`가 막혔던 셋 중 둘은 **관측의 착오**였다. 요금표 페이지를 브라우저로 열고
네트워크를 녹화하니 표를 채우는 콜이 하나 있었다:

```
POST www.hanwharesort.co.kr/irsweb/resort3/resort/getRoomPrice.do    ← 무인증 · 넷퍼넬 없음
     bp_cd=<locCd>&rm_cd=<ROOM_TYPE_CD>&sel_month=YYYYMM01
→ { amounts: [{ SESN_CD, SESN_NM, ROOM_TYPE_CD, RSRV_TYPE_CD_0..5 }],
    map: { atpTp: { "1": "무기명", "2": "기명", "3": "회원추천" } } }
```

- **`bp_cd`는 `locCd` 그대로다.** 08-26의 "`bp_cd=0100`은 빈 페이지"는 `brchCd`를 넣었기
  때문이다. 16지점 전부 `locCd`로 답한다.
- **줄이 재고 달력과 같은 `SESN_CD`로 온다.** 16지점 · 45일 예약가능 3,968밤에서 시즌
  불일치 **0**, 이름 불일치 0. 즉 "시즌 이름 → 열(월-목·금-토…)" 규칙을 우리가 만들 필요가
  없고, 가을 밖의 계절(봄 비수기 평일·주말·연휴가 이미 호텔에서 관측됐다)도 코드가 표에
  있으면 그대로 붙는다. `rs_room.do`의 `월-목 · 금-토…` 표는 **특화 객실 추가 요금표**였다.
- **값은 달마다 다르다** — 같은 `3114`(가을 비수기 일요일)가 9월 148,000 · 10월 133,000.
  그래서 요금 키는 (객실, **밤의 달**, 시즌)이다. 공표 전인 달은 `amounts: []`.
- **열 번호는 응답이 정한다**(`map.atpTp`). `HANWHA.rateFare = "기명"`은 **운영자의 답**이다
  (2026-09-24). 어휘에 그 이름이 없으면 그 지점은 요금이 없다.
- 특화 객실은 추가 요금이 **포함된** 값이다(DMO 디럭스(모던) − FAM 디럭스 = 주중 15,000 ·
  주말 50,000). `rm_cd`는 재고의 `ROOM_TYPE_CD`와 같은 코드라 107종 지도가 필요 없다.
- 표가 없는 곳: **여수 벨메르 · 브리드호텔 양양**(어느 달도 `[]`), 더플라자는 예약가능 0.

**`PP_DSCNT_RT`는 날짜·객실별 프로모션 할인율이다.** 회원 달력이 `ppDscntRt > 0`이면 버튼을
"10%/9실"로 그린다(`serviceM00.mvc` 인라인 스크립트). 무엇의 %인지는 말하지 않고 안내문은
"특화 객실 추가 금액은 할인 미 적용"이라 적는다 — 표 값에 단순히 곱하면 틀린다. 그래서
**할인이 걸린 밤이 하나라도 있으면 그 행은 요금이 없다**(실측 예약가능 밤 368/3,968 = 9%).
할인 전 금액을 그날의 요금이라 부르는 것도 틀린 안내다.

수집기는 `rates.ts`이고 모양은 오크밸리 `rates.ts` · 소노 `names.ts`와 같다:
- 달력을 받은 **같은 지점 작업 안에서** 예약가능한 (객실, 달)만 묻는다(`HANWHA.ratePool` 4).
  달력과 함께 `session.calendars`에 캐시되므로 **패스당 지점마다 한 번**이다.
  실측 지점당 0.1~0.7초 · 전체 ~380콜 — 첫 윈도우를 지배하는 것은 여전히 달력 콜(최대 5.6초)이다.
- 타임아웃은 검색 데드라인에서 유도하고 **절대 던지지 않는다.** 실패·예산 부족은
  `[hanwha] rate table incomplete`로 남고 그 행은 빈칸이다.
- `withPrices` 게이트를 타지 않는다 — 비용이 O(지점×객실×달)이지 O(행)이 아니다.
- 창마다 `[hanwha] rates { priced, unpublished, discount, season, night, noTable }`를 찍는다.
  `unpublished`·`discount`는 평상시에도 0이 아니다. **`season`·`night`·`noTable`이 늘면 조인이
  깨지기 시작한 것이다.**

실사이트 검증(2026-09-24, `CRAWL_BUDGET_MS=50000 run-crawl HANWHA hot`): 첫 창 6,624행 중 예약가능
3,966행 → 요금 3,018 · 미공표 580 · 할인 368 · 시즌 불일치 0. 손 대조: 설악 디럭스(모던) 9/30(수)
148,000 · 10/1(목) 133,000 · 10/2(연휴) 281,000, 디럭스 연휴 231,000 — 표와 일치.
DB 불변식 위반 0 — `price>0`, `price_kind` 동반·전부 `memberTable`, `available=false`에 요금 없음,
2박 총액 ≤ 1박 0건, **2박 총액 ≠ 두 밤의 합 0건**(월 경계 10/31+11/1 포함).
⚠️ 그날 첫 패스는 **요금 유무와 무관하게** 1/60 창에서 예산으로 멈췄다(A/B: 요금 끔 검색 추정
15.8초 · 켬 19.8초). 프로덕션 `crawl_logs`도 매일 6,624행 + 13,104행의 2패스라 이것은 원래 모양이다.
...
curl -s -X POST https://www.hanwharesort.co.kr/irsweb/resort3/resort/getRoomPrice.do -d 'bp_cd=0101&rm_cd=DMO&sel_month=20261001'   # 요금표 JSON (무인증)
