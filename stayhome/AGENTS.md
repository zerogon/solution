...
그래서 요금을 붙이지 않는다 — 수집이 불가능해서가 아니라 **셋 중 어느 것인지 우리가
고를 수 없어서**다. 이걸 푸는 것은 코드가 아니라 한 줄의 사실이다: 운영자가 이 회원권이
기명인지 무기명인지 말해주면 그 자리에서 붙는다. (`cdRef1`/`cdRef5`가 그걸 뜻할 수도
있지만 디코딩할 근거가 없고, 근거 없는 디코딩은 27% 틀린 금액을 발행하는 길이다.)
