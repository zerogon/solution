...
> **완료 (2026-09-13, 평소 PC).** 아래 4절의 할 일은 끝났고 이 문서는 이력으로만 남긴다.
> `db:push` → `stayhome` 머지·배포 → `variants` 조사 → `room/detail`이 **`actualMemNo`로 열려**
> 세부 축까지 한 번에 배선(`src/crawlers/sono/names.ts`), `SONO.viewNames`는 삭제했다
> (3번의 "사람이 채우는 표"가 필요 없어졌다). 실측·근거는 `CLAUDE.md` "소노 변형 수집" 절과
> `AGENTS.md` 소노 절 "### 변형과 그 이름".
