...
export const HANWHA = {
   ...
   * 한 번에 동시에 물어보는 지점 수.
   *
   * 지점이 16곳이고 콜 하나가 ~2초라, 순차로 돌면 첫 윈도우 하나가 ~32초로 이
   * 패스의 예산을 통째로 먹었다 — 그래서 콜드 패스마다 좁혀진 120행짜리 윈도우
   * 하나만 남기고 브라우저 한 벌을 다 썼다(2026-08-27 09:02 · 09:04가 그것이다).
   *
   * **16이 아니라 4인 이유**: 예약 호스트는 넷퍼넬(대기열) 뒤에 있고, 거기 걸리면
   * 증상이 에러가 아니라 **응답 없음**이다. 즉 과하게 밀어붙였을 때의 대가가
   * 느려짐이 아니라 침묵이라, 병렬도를 올리는 것은 관측하고 나서 할 일이다.
   * 4면 첫 윈도우가 ~8초로 예산 안에 들어오므로, 지금 필요한 것은 이미 얻는다.
   * 롯데가 지점 수와 같은 4를 쓰는 것과는 근거가 다르다(그쪽은 문지기가 없다).
   */
  branchPool: 4,
  ...
  timeouts: {
     ...
     */
    validateBoot: 10_000,
    /** 공개 요금 API 한 콜의 상한. 실측 평균 62ms(386콜) — 넘기면 그 달은 요금이 없다. */
    rate: 5_000,
  },

  /**
   * 공개 회원 요금표 API — **무인증**이고 `www` 호스트에 있다(넷퍼넬 없음).
   *
   * 공개 객실 페이지(`rs_room.do?bp_cd=…`)의 요금표는 JS가 이 콜로 그린다. 그래서
   * 08-26 `axes`가 `page.request`로 HTML을 받았을 때 `<table>`이 0개였다 —
   * 표가 없던 것이 아니라 **표를 채우는 콜을 아무도 안 봤던** 것이다(2026-09-24).
   *
   * `POST bp_cd=<locCd>&rm_cd=<ROOM_TYPE_CD>&sel_month=YYYYMM01` →
   * `amounts[] = { SESN_CD, SESN_NM, ROOM_TYPE_CD, RSRV_TYPE_CD_0..5 }` +
   * `map.atpTp = { "1": "무기명", "2": "기명", "3": "회원추천" }`.
   *
   * - **`bp_cd`는 `locCd` 그대로다**(16지점 전부). 08-26에 "다른 코드 체계"라 적었던
   *   것은 `bp_cd=0100`(= `brchCd`)을 넣어봤기 때문이다.
   * - **조인 키는 재고 달력이 밤마다 주는 `SESN_CD`다.** 요금표 줄이 시즌 이름을
   *   열로 뭉친 것이 아니라 **재고와 같은 시즌 코드**로 온다 — 그래서 시즌 이름 →
   *   열 규칙을 우리가 만들 필요가 없고, 처음 보는 계절도 코드가 표에 있으면 붙는다.
   * - **달마다 값이 다르다.** 같은 `SESN_CD 3114`(가을 비수기 일요일)가 9월 148,000 ·
   *   10월 133,000이다. 그래서 키는 (객실, **밤의 달**, 시즌)이다.
   * - 공표되지 않은 달은 `amounts: []`(관측: 설악 12월, 마티에 11월). 호텔 두 곳
   *   (여수 벨메르 · 브리드호텔 양양)은 **어느 달도 표가 없다** — 그 지점은 요금이 없다.
   * - 특화 객실(모던 · 유러피언 · 키즈…)은 추가 요금이 **이미 포함된** 값이다
   *   (DMO 디럭스(모던)이 FAM 디럭스보다 주중 +15,000 · 주말 +50,000).
   */
  rateApiUrl: "https://www.hanwharesort.co.kr/irsweb/resort3/resort/getRoomPrice.do",

  /**
   * 요금표의 세 열(무기명 · 기명 · 회원추천) 중 이 계정이 쓰는 것.
   *
   * **사이트가 말해주지 않는다** — 재고 응답의 `USER_CALC_GRAD_CD`는 빈 값이다.
   * 그래서 이 값은 **관측이 아니라 운영자의 답**이다(2026-09-24, 오크밸리
   * `rateFare`와 같은 질문 · 같은 답). 회원권 구성이 바뀌면 여기를 같이 바꿔야 하고,
   * 안 바꾸면 증상은 에러가 아니라 틀린 금액이다(설악 디럭스 주말: 무기명 235,000 ·
   * 기명 225,000 · 회원추천 250,000).
   *
   * 열 번호는 박지 않는다 — 응답의 `map.atpTp`가 이 이름을 번호로 바꿔준다.
   * 그 어휘에 이 이름이 없으면 요금을 만들지 않는다.
   */
  rateFare: "기명",

  /** 한 지점의 (객실, 달) 요금 콜을 동시에 몇 개 보낼지. 지점이 이미 4개씩 병렬이다. */
  ratePool: 4,
