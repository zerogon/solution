 ...
 * 반환 경로를 위해 남겨두는 시간.
 *
 * 이 값이 덮는 것은 마지막 콜이 끝나고 행을 돌려주기까지다.
 *
 * 📌 정정(2026-08-26): 여기 "upsert는 `withDeadline` 밖이고 50초와 60초 사이에
 * 여유가 있다"고 적혀 있었다. 소노가 그 여유를 다 썼다 — 한 윈도우 12,000행의 쓰기
 * 꼬리가 패스를 59.3초까지 밀어 60초 벽에 1초를 남겼다. 이제 `run.ts`가 쓰기 몫을
 * 검색 예산에서 **먼저 떼어** 두므로 그 여유는 여기서 가정할 것이 아니라 저쪽이
 * 보장하는 것이다. 이 상수는 여전히 필요하되, 근거가 다르다.
 */
