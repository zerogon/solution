 ...
 *
 * **장소 축이 하나도 없다.** 리조트·지역·지점 셋 다 클라이언트에서 `matchesPlace`가
 * 좁히므로, 서버에 보내는 것은 날짜뿐이다.
 *
 * 리조트는 2026-08-30까지 여기 있었고, 그 때문에 **똑같이 생긴 칩 두 줄의 동작이
 * 달랐다** — 리조트 칩은 결과를 흐리고 '조회' 재클릭을 요구하는데 지역 칩은 즉시
 * 반영됐다. 필드를 지우면 그 발산이 구조적으로 불가능해진다(`price`/`priceKind`를
 * 하나로 접어 내리는 것과 같은 판단). 이제 `stale`은 **날짜 전용**이다.
 *
 * 대가는 응답이 항상 전 리조트라는 것인데, 기본 상태(리조트=전체)가 이미 그랬으므로
 * 상한은 그대로다 — 한 (checkin, checkout) 쌍의 전체 행 수는 핫 윈도우 실측에서
 * 유도하면 ~650행이다.
 */
