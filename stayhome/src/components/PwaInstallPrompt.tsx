...
import { Bell, Hotel, Share, Smartphone, Zap } from "lucide-react";
...
const DISMISS_DATE_KEY = "welfarestay:install-dismissed-date";
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent side="bottom" className="gap-0 pb-4 sm:mx-auto sm:max-w-md sm:rounded-t-2xl">
        <SheetHeader className="flex flex-row items-start gap-3 pr-10">
          <div className="flex size-12 shrink-0 items-center justify-center rounded-2xl border border-border bg-primary text-primary-foreground">
            <Hotel className="size-6" />
          </div>
          <div className="flex flex-1 flex-col gap-0.5">
            <SheetTitle>홈 화면에 추가</SheetTitle>
            <SheetDescription>리조트 조회를 더 빠르게.</SheetDescription>
          </div>
        </SheetHeader>
