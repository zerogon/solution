...
/**
 * 이 등급의 행을 "예약 가능"으로 세도 되는가 — **확인된** 가용만이다.
 *
 * 요약 스탯·필터 칩 건수·지점 헤더·"예약 가능만 보기" 필터가 전부 이 술어를 쓴다.
 * 한 곳이어야 하는 이유는 필터를 켰을 때 남는 행 수가 위의 "예약 가능 N건"과 같아야 해서다 —
 * 어긋나면 사용자는 숫자와 목록 중 어느 쪽이 틀렸는지 알 방법이 없다.
 *
 * `showsPrice`와 지금 값이 같지만 합치지 않는다. 저건 "이 숫자를 그려도 되나"이고
 * 이건 "이 방이 후보인가"라서, 한쪽 규칙이 바뀔 때 다른 쪽이 딸려 가면 안 된다.
 */
export function isBookable(tone: AvailabilityTone): boolean {
