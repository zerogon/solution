 ...
 *
 * 현재 **멈춰 있다** (운영자 요청, 2026-08-13).
 *
 * 경위: 2026-08-11부터 한화 크롤러를 기다리며 멈춰 있다가, 2026-08-13에 다섯 번째
 * 리조트가 붙어 5곳을 한 번에 켰고(실사이트 검증: 핫 윈도우 60개 → 57스킵 ·
 * 요청 16번 · 13,224행 35.6초), 같은 날 다시 껐다. **크롤러가 고장나서가 아니다** —
 * 이 구분이 남아 있어야 다음 사람이 "한화가 안 되나 보다"로 읽지 않는다.
 *
 * ## 다시 켜는 법 / 멈추는 법
 *
 * 이 상수 하나만 바꾸고 배포한다. 그게 전부다 — DB도 Inngest 대시보드도
 * 건드릴 것이 없다. `Resort.active`는 다섯 곳 모두 true로 남아 있으므로
 * 켜는 즉시 5곳이 팬아웃된다.
 *
 * ## 왜 크론 트리거를 지우지 않는가
 *
...
export const SCHEDULED_CRAWL_PAUSE_REASON =
  "운영자 요청으로 중단 (2026-08-13). 크롤러 5곳은 검증 완료 상태 — 고장이 아니다";
