...
- **응답에서 사라진 행은 지운다** (`removeVanishedRows`, `run.ts`). upsert만으로는
  ...
  · 실측(2026-08-24): 롯데 3,742→3,739(속초 3행), 소노 18,018→18,010, 리솜·한화 변동 0,
    크롤하지 않은 윈도우는 무영향.
- **`crawl-resort`의 동시성은 리조트별 1 + 전역 2다.** 앞의 것은 한 리조트의 두 패스가
  `ResortSession`의 storageState 행을 두고 경쟁하지 않게 한다. **뒤의 것이 없으면
  콜드 Vercel 인스턴스 하나에 브라우저 다섯 벌이 얹힌다** — 2026-08-25 09:00 실행에서
  셋이 `spawn ETXTBSY`로 죽고 한화가 나중에 `/tmp` 고갈로 재시도를 소진했다.
  · 전역 1이 아니라 2인 이유: 재시도 백오프 중인 리조트가 나머지 넷의 머리를 막으면
    안 된다(그날 한화가 09:05~09:08을 혼자 썼다). 대가는 스윕 전체의 벽시계뿐이고,
    하루 1회 배치에서 그건 대가가 아니다.
...
- **크롤 한 번이 `/tmp`를 다 쓰면 다음 크롤이 죽는다** — 2026-08-23 실측·수정.
  ...
  · **그리고 청소로는 풀리지 않는 두 번째 축이 있다 — 동시성**(2026-08-25 실측).
    ...
    `sweepStaleProfiles`는 `/proc`을 물어 사용 중 프로필을 **올바르게 건너뛰므로**
    이 압박 앞에서 회수할 것이 하나도 없다. 525MB짜리 `/tmp`에 크로미움 추출본 하나와
