...
## 마감일 계산기 (2026-08-30, 2026-09-01 인라인 전환)

데스크톱 사이드바 하단의 위젯. 기준일을 고르면 **주말과 공휴일을 뺀 10영업일 전**을
알려준다(예약 마감일 역산). `AppShell` → `DeadlineCalculator` + 지연 로드되는
...
- **규칙: 기준일에서 하루씩 뒤로 가며 영업일만 센다.** 따라서 기준일 자신은 **절대 세지
  않고**, 답은 언제나 영업일이며, 기준일이 토요일이든 공휴일이든 특례가 없다.
  골든 케이스는 `2026-08-29(토) → 2026-08-14(금)`(주말 4 · 8/17 하나)이고, 단순 -10일인
  `08-19`와 **5일 차이**다. 이 표가 `business-days.ts` 헤더에 있다.
- **"모른다"를 "휴일 아님"으로 읽으면 안 된다** — 그래서 판정자가 `(iso) => boolean`이
  아니라 `covers`/`isHoliday`/`nameOf`를 가진 `HolidayOracle`이다(`src/lib/holidays-kr.ts`).
  실패 방향이 나쁜 쪽이라서다: 공휴일을 적게 세면 건너뛰는 날이 줄어 **마감일이 뒤로
  밀리고**, 사용자는 시간이 더 있다고 믿는다. 판정할 수 없으면 날짜 대신 **"계산 불가"**다.
...
- **⚠️ 개수 바닥이 이 기능의 새 불변식이다.** 구글이 `공휴일` 표기를 바꾸면 필터가 아무것도
  매치하지 않고 → "휴일 없는 해"처럼 보이고 → 주말만 건너뛴 **4~5영업일 늦은 날짜**가 나간다.
