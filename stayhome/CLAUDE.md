...
## 소노 변형(뷰 축) 수집 — 접힌 행 아래 세부 목록 (2026-09-11, 구현 완료 · 실사이트 조사 대기)

운영자 질문에서 시작했다: 소노는 객실유형 아래에 세부 타입이 있다(소노벨 청송 → 리조트 패밀리 →
스탠다드/파크뷰 → 더블취사/트윈취사/더블클린/트윈클린). 그걸 수집해 보여줄 수 있는가.

**답은 두 층으로 갈린다.** 뷰 축(스탠다드/파크뷰)은 **이미 받고 있는** `room/list/pc` 응답에
변형(`rmTypeCd`)마다 `viewCd`·`rsvStatusCd`·`rsvRmCnt`로 들어 있다 — 추가 콜 0. 다만 **코드만
있고 이름이 없다**(`viewNm` 없음, 15키 전수). 세부 축(더블취사 …)은 객실 선택 단계의
`room/detail`(`bedNm`·`cookNm`)에만 있고, 그 콜은 08-31 직접 호출이 `이용회원번호는 필수입니다`로
거절돼 **아직 재현되지 않았다**(`AGENTS.md` 소노 절).

**운영자 결정 둘**: ① 접힌 행은 그대로 두고 **행 아래에 세부 목록을 펼친다**(라벨 · 잔여 · 상태) —
행 수·유니크 키·필터 칩 건수·수동 요금 조인 키를 바꾸지 않는다. ② **뷰 축을 먼저**, 세부 축은
조사에서 `room/detail`이 열릴 때만 같은 그릇에 라벨을 늘려 붙인다.
...
- **모르는 `viewCd`에 이름을 지어내지 않는다.** `SONO.viewNames`(빈 표, `"01"` 전역 또는
  `"66:01"` 지점 한정 키)에 없으면 코드가 그대로 라벨이고, `search.ts`가 배치당 한 번
  `[sono] unnamed viewCd`를 남긴다. 오크밸리 미등록 객실유형과 같은 규칙. 같은 라벨의 변형이
  한 행에 둘이면(한 뷰 안에 `rmTypeCd`가 여럿 = 세부 축이 이미 갈라진 경우) `라벨 (코드)`로
  구별한다 — 합치면 "44실 남음" 거짓이 다시 생긴다.
...
### 아직 안 한 것 — 실사이트 조사 (`debug-sono.ts variants`)

스텝은 작성됐고 **돌리지 못했다**: 이 작업 환경에 `.env`가 없어 DB(자격증명)에 닿지 못했다
(`.env.example`만 있음). 그래서 지금 배포하면 소노 세부 목록의 라벨은 **`viewCd` 코드 그대로**다
(의도된 강등이고 로그에 남는다). 조사가 답해야 할 것:

1. `viewCd`가 전역 어휘인가 지점별인가 → `viewNames` 키 모양. 그리고 그 이름(헤드 브라우저의
   객실 선택 화면과 잔여 수 대조로 확인해 표에 넣는다 — 사람의 답이라는 사실을 주석에).
2. `rmTypeCd` → (roomTypeCd, viewCd, pyeongCd) 함수성 · 역방향 1:1 · 날짜/월 간 안정성 ·
   `rmTypeCode` 요청 필드의 뜻 · **2박 접기 오판 행 수**(위 정확성 수정의 근거 숫자).
3. `viewCd` 이름표 출처 — `userinfo` 전 필드(마스킹) · SPA 번들의 코드표 엔드포인트 ·
   `room/detail` 재현(선행 콜 순서 재생 / 본문 보강 / 새 컨텍스트 — H1·H2·H3 판정표).
4. 열렸다면 `room/detail`의 34키·`viewList` 키(`viewNm`?)·`bedNm`/`cookNm` 어휘와 null 비율 ·
   `rmTypeCd` 조인율. 5. 비용(`storeCdList` 1/4/8, 응답이 달 단위인가).
   **세부 축 GO 조건 넷**: DOM 없이 `page.request`로 열림(선행 ≤3콜) · 8지점/콜 또는 월 단위 ·
   조인 ≥99% · 이름 non-null ≥95%. 하나라도 아니면 뷰 축만 배포하고 실패 기준을 적는다.

돌리기: `.env` 복원 후 `npx tsx scripts/debug-sono.ts doLogin && npx tsx scripts/debug-sono.ts variants`,
그다음 `npm run db:push`(컬럼 추가) → `run-crawl.ts SONO "소노벨 청송"` → 검증 SQL은
`AGENTS.md` 소노 절 "### 변형은 뷰 축이고 이름은 응답에 없다"에.

오프라인 검증(2026-09-11): 합성 페이로드로 파서 불변식 확인 — 1박은 옛 접기와 판정 동치,
2박 갈림 케이스에서 행은 남고 `available`만 false, 행 수 불변, 같은 라벨 충돌 시 `(코드)` 부기,
`remaining` = 밤 최소, 변형 1개 행도 길이 1 배열. `tsc`·`eslint`·`next build` 통과, 번들 유출 0.
