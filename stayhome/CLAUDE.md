...
- **`crawl-resort`의 동시성은 리조트별 1 + 전역 1이다** (2026-08-26에 전역 2 → 1).
  ...
  · 2를 고른 근거("재시도 백오프 중인 리조트가 나머지 넷의 머리를 막으면 안 된다")는
    ...
    이미 끝나 있었다.**
  · **1이 청소 기구를 살려낸다.** `sweepStaleProfiles`는 `/proc`을 물어 살아 있는
    프로필을 올바르게 건너뛰므로, 동시 크롤이 있는 한 회수할 것이 구조적으로 0이다.
    직렬화해야 매 launch 전 sweep이 직전 크롤의 잔해를 실제로 회수한다.
  · 대가는 벽시계뿐이다(2레인 ~6.5분 → 1레인 ~11분). 신선도 임계가 26시간인
