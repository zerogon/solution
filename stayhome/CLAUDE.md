...
- 셸 지오메트리는 매직넘버가 아니라 토큰: `--app-sidebar-w` / `--app-sidebar-rail-w` /
  ...
  · **앞의 것과 `--app-content-w`는 한 쌍이다** — 합이 1664px로 고정돼야 1920px에서
    사이드바-본문 관계가 유지된다. 2026-09-01에 마감일 계산기 달력이 사이드바 안으로
    들어오면서 16→**18rem**이 됐고, 그만큼 content-w를 88→**86rem**으로 줄였다.
    한쪽만 고치면 본문이 밀려 72rem 시절의 증상이 그대로 돌아온다.
...
- **Phase E-2 (완료, 2026-08-05)**: 조회 UX 개선. 주간 날짜 스트립 → **월 캘린더**
  ...
  (`--app-content-w` 72→88rem — 1920px에서 사이드바-본문 공백 288→160px. 현행 값은
  86rem이다 — 2026-09-01 사이드바 확대와 한 쌍으로 줄었다),
...
- **"예약 가능만 보기"(2026-09-15)는 결과 목록의 표시 필터다** — 서버 축도 쿼리 키도 아니고
  `/api/inventory`·`sw.js`(`CACHE_VERSION`)는 무변경. 판정은 `availability-tone.ts`의 `isBookable`
  (확인된 가용 = `available`·`closingSoon`, **확인 필요는 숨김** — 운영자 결정) 한 곳이고, 요약 스탯·
  칩 건수·지점 헤더도 같은 술어를 쓴다. 그래서 켰을 때 남는 행 수 = 요약의 "예약 가능 N건".
  요약과 지점 헤더는 **필터 전 행**으로 세고 좁히는 것은 목록뿐이다. 단 헤더의 요금 종류 라벨은
  목록(`listed`)에서 구한다(숨긴 매진 행의 수동 요금이 라벨만 남기지 않도록). 상태는 브라우저별
  localStorage(`use-available-only.ts`, 기본 꺼짐).
...
- **재고 upsert는 다중행 `ON CONFLICT` 한 문장**(`upsertInventory`의 `$executeRaw`).
  ...
  · 행당 바인드 파라미터가 17개(2026-09-11 현재)라 Postgres 상한 65,535에 걸린다.
    `UPSERT_CHUNK_ROWS`(1,000행)로 끊는다 — 소노 한 콜이 ~3,900행이라 12개 시절엔
    상한까지 1.4배밖에 안 남았었다.
...
| --- | --- | --- | --- | --- |
| **롯데** | **있음 · 수집 중** | 이미 받고 있는 `roomList` 안 | `roomAvgAmt`(1박 평균). **2026-09-01부터 회원가** — `memberNo`+`ownType`을 실으면 같은 콜이 회원 요금을 답한다(BAR 대비 −14~−27%). 세션이 없으면 **공시가로 강등**하고 라벨도 같이 내려간다 | **0** — 지점당 콜이 둘이지만 **동시에** 나가 벽시계가 그대로다 |
| **리솜** | **있음** | 별도 `roomReservation/stockPrice` | `totalRmAmt`(총액) + `rmAmtList`(밤별) + `totalCmpnyRmAmt`(회사지원금). **회원가** | **행마다 1콜**, 0.2~1.8초 |
| **소노** | **있음 · 수집 중**(최신화만) | 객실 선택보다 **한 홉 뒤** — `room/detail/price` | `totAmt`(총액, 할인 반영) + `daysAmtInfoList`(밤별 **할인 전**) + `seasonCd`. **회원가**(`originAmt − levelAmt`). **행이 아니라 `variants[]`에 붙는다** | **변형마다 1콜**, 166ms (한 지점·하루 13콜 2.4초) |
| **오크밸리** | **있음 · 수집 중** | 예약 API 아님 — **공개** `api/v1/village`의 `roomPriceTable` | 회원 요금표 + **시즌 달력**. `memberTable`(계산값) | **0** — 패스당 공개 GET 1회 |
| **한화** | **있음 · 수집 중** | 예약 API 아님 — **공개** `getRoomPrice.do`(요금표 페이지가 그리는 JSON) | 회원 요금표 **기명** 열. 줄이 재고와 같은 `SESN_CD`로 온다. `memberTable`(조인값) | 지점당 (객실×달) 콜, **0.1~0.7초** — 패스당 한 번 |
...
**결정 4 (2026-09-01): 롯데는 회원가로 바꾼다 — 문을 여는 것은 쿠키가 아니라 파라미터 둘이다.**
운영자가 "사이트에 로그인하면 객실 목록에서 이미 다른 금액이 보인다"고 알려온 것이 출발점이다.
조사(`debug-page.ts member`)가 셋을 측정했다 — ① **로그인만으로는 이 요청의 답이 한 글자도
바뀌지 않는다**(인증/익명 4지점 71행: 다른 셀 0), ② 사이트는 로그인하면 `memberNo`와
`ownType`을 다르게 채운다(비로그인 `""`/`1` → 로그인 10자리/`5`), ③ 그 둘을 실으면 요금이
**−14.2 ~ −26.6%**로 바뀐다. `memberNo`만으로는 0행, `ownType`만으로는 BAR와 동일이다.

- **`ownType`은 상수가 아니라 유도값**이다(`member.ts`의 `ownTypeOf`). 사이트 번들이 어휘를
  직접 적어 두었고(`1` 기명 · `2` 지인 · `5` 무기명) 계정의 `registerCd`가 고른다.
  오크밸리 `rateFare`가 운영자의 답을 상수로 박아야 했던 자리와 **정확히 같은 질문인데,
  롯데는 사이트가 답한다.** 모르는 코드에는 값을 지어내지 않고 공시가로 물러난다.
- **열쇠는 세션에서 패스당 한 번 읽는다** — `/common/login/user`의 분양회원(`membershipType:"R"`)
  엔트리. 소노 `memNo`·한화 `sCustNo`와 같은 규칙이고, config에 박으면 계정을 갈아끼웠을 때
  증상이 에러가 아니라 **남의 회원 요금**이다. ⚠️ 그 응답에는 회원사명·담당자명·이메일이
  평문으로 들어 있어 `member.ts`는 읽는 칸만 선언하고 회원번호는 로그에 남기지 않는다.
- **재고와 요금의 출처가 갈렸다.** 회원 트랙은 BAR의 **부분집합**이라(부여 23/25 · 제주 13/14 ·
  김해 18/20) 그걸로 재고를 만들면 예약 가능한 방이 화면에서 사라진다. 그래서 지점당 콜이
  둘이고, **재고는 BAR가 요금은 회원 트랙이** 답한다. 회원 트랙에 없는 방은 요금이 **빈칸**이다 —
  BAR 값을 대신 넣으면 한 섹션 안에 회원가와 공시가가 행 단위로 구별되지 않게 섞인다.
- **라벨은 응답이 아니라 출처가 정한다.** `memberType`이 회원 트랙에서도 전 객실 `""`라
  응답 안에는 트랙을 신고하는 칸이 없다. "응답에서 유도한다"는 첫 설계가 성립하지 않은
  자리이고, 그래서 `parse.ts`가 **어느 페이로드에서 온 숫자인지**로 `kind`를 정한다.
- **콜 타임아웃이 `ctx.deadlineAt`에서 유도된다**(한화·오크밸리와 같은 계보). 이 사이트의
  지연이 널뛰기 때문이다 — 처음 묻는 (지점, 날짜)는 **18초**까지 가고 다시 물으면 1초다.
  상한만 쓰면 낙오자 하나가 부분 반환을 `DeadlineExceeded`로 바꿔 패스가 커밋한 행까지
  FAILED로 신고한다.
- 화면·API·스키마는 **아무 변경도 없다.** `PriceKind`에 `member`가 이미 있었고(리솜이 쓴다),
  `price_kind`는 text라 마이그레이션이 아니며, `BranchResultSection`은 한 섹션의 종류를
  집합으로 구해 그린다. `sw.js`의 `CACHE_VERSION`도 그대로다 — 응답 shape이 안 바뀌었다.
...
~~**한화는 멈췄다 (2026-08-26 `axes`).**~~ — **2026-09-24에 풀었다. 아래 결정 5.** 불가능해서가 아니라 배선 하나가 아니라서다.
...
**결정 5 (2026-09-24): 한화는 공개 요금표 JSON으로 붙인다 — 기명, 정기 수집.**
세 장벽 중 둘은 **없던 장벽**이었다. 요금표 페이지는 JS가 `POST getRoomPrice.do`
(`bp_cd`·`rm_cd`·`sel_month`, **무인증**)로 그리고, 그 응답이 `SESN_CD`·`ROOM_TYPE_CD`로
온다 — 재고 달력과 **같은 코드**다. 08-26이 "다른 코드 체계"라 본 `bp_cd`는 그냥 `locCd`였다
(`0100`=`brchCd`를 넣어봤던 것). 그래서 107종 객실 지도도, 시즌 이름→열 규칙도 필요 없다.
남은 하나(회원 등급)는 운영자가 답했다 — **기명**(`HANWHA.rateFare`, 오크밸리와 같은 답).
수집기는 `src/crawlers/hanwha/rates.ts`, 상세는 `AGENTS.md` 한화 "### 요금표는 JSON이었다".
- **밤마다 (객실, 밤의 달, 그 밤의 `SESN_CD`)로 찾아 더한다.** 같은 시즌 코드가 달마다
  값이 다르다(설악 디럭스(모던) 가을 비수기 일요일 9월 148,000 · 10월 133,000).
- **프로모션 할인(`PP_DSCNT_RT`)이 걸린 밤은 요금을 만들지 않는다.** 사이트 달력의
  "10%/9실"이고, 무엇에 대한 %인지 사이트가 말하지 않는다. 실측 예약가능 밤의 9%.
- 호텔 두 곳(여수 벨메르 · 브리드호텔 양양)은 요금표를 **공표하지 않아** 빈칸이고,
  아직 공표되지 않은 달(관측: 설악 12월 · 마티에 11월)도 빈칸이다.
- `price.ts`·스키마·`/api/inventory`·`sw.js` **무변경**(`memberTable`이 이미 있다).
  ⚠️ 한화 지점에 입력해 둔 **수동 요금은 이제 자동 요금에 가려진다**(위 "수동 요금" 절이 예고한 그 일).

~~**소노는 하지 않는다.**~~ — **2026-09-21에 전제가 바뀌었다. 소노에는 요금이 있다.**
`debug-sono.ts prices`가 `POST memberReservation/room/detail/price`를 열었다. 막고 있던
것은 본문의 **`rmTypeCd`** 한 칸이었고, 이번에는 사이트가 빠진 칸의 이름을 직접 댔다
(`storeCd 는 필수값입니다` → `rmTypeCd 는 필수값입니다` → 200). 08-25 `flow`가 스스로
"이 조사는 객실 선택까지다"라고 적어둔 그 경계 **바로 뒤**에 있었다.
· 값은 회원가다 — `originAmt 171,000 − levelAmt 25,000 = totAmt 146,000`. 숙박 **총액**을
  주므로 곱하지 않는다(1·2·3박 146,000 → 292,000 → 438,000). ⚠️ `daysAmtInfoList`의 밤별
  금액은 **할인 전** 값이라 그 합(171,000)은 총액이 아니다.
· ⚠️ **예약대기·마감임박 변형에도 값이 온다**(W 392,000). 응답에 리솜 `isPossible` 같은
  자기 부인 칸이 없어 가용성 게이트는 우리 쪽에 있어야 한다.
· **배선했다(2026-09-21) — 변형별로, 최신화 경로에만**(`src/crawlers/sono/prices.ts`).
  조인 키가 `rmTypeCd`(변형)이고 **같은 행의 변형끼리 요금이 다르므로**(실측 한 행에서
  97,000원과 137,000원) 행에 하나를 고를 수 없다. 09-11에 생긴 `variants` 컬럼이 그 자리다 —
  행의 `price`/`price_kind`는 소노에서 **영원히 null**이고, 그래서 `run.ts`의 `pricedRows`가
  행과 변형을 **함께** 센다(`hasPrice`). 안 그러면 이 리조트에서 그 숫자가 영원히 0이라
  "예산에 걸려 일부만 붙었다"가 보이지 않는다.
  · 비용이 형태를 정했다 — `storeCd`·`rmTypeCd`가 단수라 **배치가 불가능**하고(배열은 500),
    핫 윈도우 전체가 ~30,000콜 ≈ 80분이다. 최신화 한 번은 **2.4초**(13콜). 리솜과 같은
    2중 게이트(`withPrices && branches.length === 1`).
  · 스키마·`/api/inventory`·`run.ts` upsert는 **무변경**. 변형 객체 모양이 바뀌었으므로
    `sw.js` `CACHE_VERSION` v5 → **v6**.
· 옛 문장이 막힌 이유로 들던 "접기"는 **요금에 대해서는 지금도 참이다**(실측 150그룹 중
  90그룹이 2개 이상 접힘). 달라진 것은 그 접기가 이제 **극복 가능한 설계 문제**이지
  "금액이 없다"가 아니라는 점이다. 상세는 `AGENTS.md` 소노의 "### 요금은 있었다".
...
### 나머지 넷을 다 재봤다 (2026-08-31) — 둘은 붙였고, 둘은 붙이지 않는다

| 리조트 | 정원이 있나 | 어디에 | 붙였나 | 비용 |
| --- | --- | --- | --- | --- |
| **리솜** | **있음(기준·최대)** | 이미 부르고 있는 `calendarRooms`의 `initPersCnt`/`maxPersCnt` | **예** | **0** |
| **오크밸리** | **있음(기준·최대)** | 공개 `api/v1/room?idCondo=`의 `standardCount`/`maxCount` | **예** | 패스당 조회 1벌 |
| 한화 | **기준만** | 공개 `rs_room.do`의 `<dt>객실정원</dt>` | 아니오 — 최대가 없다 | — |
| 소노 | 없음 | 재고 15키 · 공개 객실 61개의 `personCnt` 전부 null | 아니오 | — |

이 조사에서 다시 확인된 것 넷:

- **필드가 있다고 값이 있는 게 아니다 — 이번에만 세 번 나왔다.** 리솜 `allCondos`의
  `initPersCount`/`maxPersCount`는 17엔트리 전부 `0`이고, 진짜 값은 **이름이 한 글자
  다른**(`Count` → `Cnt`) `calendarRooms`의 `initPersCnt`/`maxPersCnt`에 있다.
  소노 공개 객실 페이지의 `personCnt`·`peopleCntText`는 61객실 전부 `null`이다.
  그전에 리솜 `rmAmt`가 506엔트리 전부 `"0"`이었다. 그래서 가드는 `_shared/occupancy.ts`
  한 곳에 있고 `< 1`을 거른다 — 이름만 보고 배선했다면 전 객실이 0명으로 나갔을 것이다.
- **"재고가 없다"가 "정원도 없다"는 뜻이 아니었다.** 오크밸리 `api/v1/room`은 이 저장소가
  한 번도 부른 적 없는 무인증 엔드포인트다. 옆의 `api/v1/condo`를 "인벤토리 API로 착각하지
  말 것"이라고만 적어둔 주석이 그 문을 6개월간 가렸다. 파라미터(`idCondo`)는 추측이 아니라
  **마케팅 SPA 번들의 요청 조립부에서 읽었다** — 추측하면 400이 `"콘도 정보가
  누락되었습니다"`로만 와서 이름이 틀린 것과 값이 틀린 것이 구별되지 않는다.
- **요금에서 막힌 축이 정원에서는 안 막힌다.** 오크밸리 요금표는 `31평 (일반)`과
  `31평 (노블)`이 20% 달라 어느 쪽도 고를 수 없었는데(`config.rateRows`의 AP·BU가 비어
  있는 이유), 정원은 그 변형들이 **전부 같은 값**이라 9종이 다 붙는다. 같은 크롤러에서
  요금은 7종, 정원은 9종이다.
- **소노를 막는 것은 접기가 아니었다.** 이 문서는 "평형·뷰 변형을 접어서 단일 정원이
  없다"고 적고 있었는데, 실측하니 **570그룹 중 평형이 섞인 그룹은 0개**다(접힘은 전부
  뷰 축이다). 소노가 빈 이유는 접기가 아니라 **값이 아무 데도 없어서**다.

**한화는 값이 있는데 붙이지 않는다** — 이 조사에서 가장 아까운 자리다. 공개 페이지가
`(bp_cd, rm_cd)`마다 `객실정원`을 서버 렌더로 주고, `rm_cd`가 재고의 `ROOM_TYPE_CD`와
**1:1로 일치**한다(설악 15종 실측). 그런데 **최대인원이 없다** — 최대는 일부 지점의
산문 각주("정원 외 최대 2명까지 추가 입실이 가능합니다", 4개 지점 표본 중 2곳)뿐이고,
지점마다 있다 없다 하는 안내문을 방마다의 측정값으로 승격시킬 수는 없다.
계약이 "둘 다이거나 둘 다 아니다"인 이유가 정확히 여기에 있다: 기준만 써서 "4인"이라
그리면, 6인이 잘 수 있는 방을 6인 가족을 찾던 담당자가 후보에서 뺀다.
**운영자 결정(2026-08-31)**이고, 최대인원 출처가 나오면 배선은 `parse.ts` 한 곳이다.
· 부수 소득으로 **요금 조사가 멈춰 섰던 장벽 둘이 풀렸다** — `bp_cd ↔ 지점` 지도와
  `rm_cd ↔ ROOM_TYPE_CD` 1:1. 남은 것은 회원 등급(무기명/기명/회원추천) 하나다.
  상세는 `AGENTS.md`의 한화 "### 인원".

### 정정 — "정적 소스면 컬럼이 아니다"는 틀렸다

이 절은 원래 이렇게 적고 있었다: *"정원이 재고 응답이 아니라 정적 소스에만 있다면 그건
`ResortInventory` 컬럼이 아니다 — 정기 크롤이 매번 그 컬럼을 null로 덮는다."*

**논거가 성립하지 않는다.** null로 덮이는 것은 크롤러가 그 값을 *다시 붙이지 않을 때*뿐이고,
`oakvalley/rates.ts`가 이미 반대 사례다 — 공표된 표를 패스당 한 번 받아 모든 행에 붙인다.
정원도 같은 모양(`oakvalley/occupancy.ts`)이라 매 패스 다시 쓰이고, COALESCE 금지와
충돌하지 않는다. **경계는 "정적이냐"가 아니라 "매 패스 다시 붙이느냐"다.**

원래 경고가 겨냥한 것은 남는다 — **한 번 쓰고 다시 안 붙이는 값**을 이 컬럼에 두면 안 된다.
그건 재고 행의 속성이 아니라 객실 마스터의 일이고, 그때는 카탈로그 모듈이 사본 드리프트를
막는 방식을 먼저 볼 것(`resort-catalog.ts` 주석).

### 가드는 한 곳에 있다

`occupancyOf`/`personCount`는 `src/crawlers/_shared/occupancy.ts`로 올렸다(호출자가 셋).
`_shared/pool.ts`·`_shared/branches.ts`와 같은 판단이고, 근거도 같다 — 크롤러마다 가드를
따로 두면 어긋났을 때 증상이 에러가 아니라 **리조트마다 다른 기준으로 판정된 정원이
조회 화면의 한 열에 나란히 서는 것**이다.

**`sw.js`의 `CACHE_VERSION`은 올리지 않았다.** 규칙은 "응답 shape이 바뀌면"이고,
`occupancy`는 v4에서 이미 shape에 있다. 바뀐 것은 그 칸이 차는 리조트 수뿐이다.

## 수동 요금 입력 (2026-09-02)

사이트가 금액을 주지 않는 객실에 **운영자가 1박 단가를 직접 넣는다.** 조회 결과 행에서
바로 입력하고, 넣은 값은 그 (지점, 객실유형)의 **모든 날짜·박수** 조회에 붙는다.
관리 화면은 `/admin/rates`(내비 "요금"), CLI는 `scripts/set-room-rate.ts`.

이 기능이 생긴 이유는 요금 수집이 리조트마다 다른 벽에서 멈춰 있기 때문이다 —
소노·한화는 사이트가 금액을 아예 안 주고(각 절의 "### 요금"), 리솜은 행 하나에 콜 하나라
정기 수집 예산에 못 들어가며, **롯데·오크밸리조차 전 객실이 차지 않는다**(아래).
수집을 더 미는 것은 벽마다 다른 작업이라, 사람이 아는 값을 넣을 자리를 만들었다.

- **입력 단위는 (지점, 객실유형)당 1박 단가 하나**이고 요일·시즌 축이 없다(운영자 결정).
  즉 **금·토가 더 비싼 실제 요금표를 평일 값으로 주장한다** — 오크밸리 공표 요금표 기준
  힐스 25평이 수 69,000 / 금 90,000 / 토 104,000이라 토요일은 최대 −34%다. 화면이 그 숫자를
  `수동 입력`이라 부르는 것이 유일한 방어이고, 그래서 **라벨을 지우면 안 된다.**
  · 막다른 길은 아니다: 유니크 키를 그대로 두고 `perNightFri`/`perNightSat`를 nullable로
    더한 뒤 `withManualRates`가 밤을 걸으면 된다(`oakvalley/rates.ts`가 이미 그 모양이다).
    기존 행은 두 칸이 null이라 지금 동작 그대로 남는다.
- **자동 수집 요금이 우선이다.** 수동값은 `row.price == null`인 행에만 붙고 입력 버튼도
  거기에만 뜬다(`withManualRates`의 조기 반환 한 줄). 사이트가 답한 값 위에 사람의 기억을
  덧씌울 자리를 만들지 않는다.

### 별도 표인 이유 — `ResortInventory.price`에 쓰면 다음 크롤이 지운다

`run.ts`의 upsert가 `price = EXCLUDED.price`로 덮으므로, 요금 없이 도는 다음 크롤이
반드시 지운다. 그건 버그가 아니라 **"요금의 나이 = `synced_at`"**을 지키는 불변식이고
(그 함수의 COALESCE 금지 주석), 수동 요금은 그 등식 **밖**에 서야 한다 — 자기 나이는
`updatedAt`이고 크롤과 무관하게 늙는다.

`resort_room_rates`는 `ResortBranchExclusion`과 같은 계보다 — 카탈로그의 사본이 아니라
**덧붙이는 규칙**이고, 이름이 어긋나면 에러가 아니라 **무동작**(그 행에 요금이 안 붙는다)이다.
실패 방향이 "틀린 요금이 보임"이 아니라 **"요금이 안 보임"**이다.
· 다만 지점 제외와 갈리는 곳이 하나 있다: **객실유형에는 대조할 카탈로그가 없다**
  (사이트가 정하고 한화만 107종). 그래서 `excludeProperty` 같은 생성 가드를 둘 수 없고,
  대신 **입력 다이얼로그가 항상 실제 조회 행에서 열려 값을 그대로 싣는다** — 사람이
  타이핑할 자리 자체를 없앴다. `/admin/rates`가 요금을 **만들지 않는** 이유도 이것이다.

### 병합은 클라이언트에서 — `/api/inventory`는 한 줄도 안 바꿨다

`/api/inventory`는 서비스워커가 SWR로 캐시하는 **유일한** 엔드포인트다. 요금을 거기 실으면
방금 저장한 값이 캐시본에 가려 "저장했는데 안 뜬다"가 되고, 그걸 피하려면 최신화 버튼이
쓰는 `x-fresh` 왕복을 입력할 때마다 돌려야 한다. 별도 `GET /api/room-rates`(날짜 파라미터
없음, 쿼리 키 상수)를 두면 그 문제가 **생기지 않는다.**
- 병합은 순수 함수 하나에 가둔다(`src/components/search/manual-rates.ts`). 자리는
  `matchesPlace` **뒤** · `groupByBranch` **앞**이고 `useMemo`여야 한다 — 매 렌더 새 배열이면
  `counts` memo가 무효화되고 거기 필터 칩의 예약 가능 건수가 걸려 있다.
- **`sw.js`는 손대지 않았다.** `/api/room-rates`는 `shouldBypass()`에도 SWR 목록에도 없어
  그냥 네트워크로 나가고(`/api/holidays` 선례), `/api/inventory` 응답 shape이 그대로라
  `CACHE_VERSION`도 v4 그대로다.
- 저장·삭제 후 갱신 수단은 `invalidateQueries(["room-rates"])` **하나뿐**이다. 서버 액션의
  `revalidatePath("/admin/rates")`는 관리 화면용이고, `properties.ts`처럼 `/`까지 무효화해도
  아무 일도 일어나지 않는다(조회 화면은 요금을 서버에서 읽지 않는다).
- ⚠️ `enabled: committed != null` 게이트를 지우지 말 것 — 쿼리 키가 상수라 게이트가 없으면
  조회를 누르지 않은 세션에도 요청이 조용히 붙는다(마감일 계산기가 같은 실수를 했다).
  `staleTime`은 프로바이더 기본 30초가 아니라 **5분**이다.

### `showsPrice` 게이트를 타지 않는다 — 그리고 그 대가를 갚는다

`showsPrice`(낡은·매진 행의 요금을 가림)의 근거는 "요금은 행과 같은 문장으로 쓰이므로
행보다 낡을 수 없지만, 행 자체는 낡을 수 있다"이다. 수동 단가는 **그 문장 밖에서** 쓰였다 —
행이 13일 됐다는 사실이 이 숫자에 대해 아무 말도 하지 않는다. 그래서 낡은 행에서도,
매진 행에서도 그린다(담당자가 다른 날짜를 찾을지 판단할 때 쓰는 숫자다).

⚠️ **`occupancy`의 면제와 논거가 같지 않다.** 저기는 "정원은 변하지 않는다"인데 요금은
변한다 — `PriceKind`에 `memberTable`이 따로 있는 이유가 그것이고("표는 개정되고, 개정은
통보되지 않는다"), 사람이 타이핑한 단가는 그보다도 신뢰 등급이 낮다. 그래서 면제의 대가를
**자기 나이를 자기가 말하는 것**으로 갚는다: 행 `title`에 `1박 N원 × M박 · X일 전 입력`,
`/admin/rates`에 `updatedAt`. 이 장치가 없으면 6개월 된 단가와 어제 넣은 단가가 픽셀 단위로
같아진다. 이 저장소에서 "요금의 나이 = `synced_at`" 등식 밖에 서는 **최초의 요금**이다.

### 행마다 출처를 말한다 — 섞이는 것을 막을 수 없기 때문에

요금 종류 라벨은 섹션 헤더에 한 번만 그려지는데(`BranchResultSection`), **한 지점 안에서
자동 요금 행과 수동 요금 행이 실제로 섞인다.** 빈칸은 소노·한화만의 일이 아니다:

- **롯데** — 회원 트랙이 BAR의 부분집합이라 거기 없는 방이 빈칸이다(부여 23/25 · 제주 13/14 ·
  김해 18/20).
- **오크밸리** — 밸리 31평·46평은 요금표가 `(일반)`/`(노블)`로 갈려 일부러 안 붙인다(9종 중 7종).
- **리솜** — 최신화를 안 누른 윈도우가 통째로 빈칸이다.

헤더는 "회원가 · 수동 입력"이라고 둘 다 말할 뿐 **어느 행이 어느 쪽인지는 말하지 못한다.**
그래서 수동인 행이 금액 앞에 `수동` 두 글자를 단다. 섞이지 않게 만들 수 없으므로 구별되게 만든다.
· 그리고 **렌더 게이트와 헤더의 라벨 집합이 같은 술어**(`showsRowPrice`)를 써야 한다.
  어긋나면 증상이 에러가 아니라 "라벨 없는 숫자" 또는 "숫자 없는 라벨"이다.

### 그 밖

- **`PriceKind`에 `manual`이 늘었다** — `PRICE_KIND_LABEL`·`isPriceKind`와 **셋을 같이**.
  다만 이 값만은 `resort_inventory.price_kind`에 **절대 저장되지 않는다**(크롤러가 발행하지
  않는다). 즉 그 유니언은 이제 "DB에 저장되는 어휘"가 아니라 **"화면이 그리는 요금의 종류"**다.
  회귀 검사: `SELECT count(*) FROM resort_inventory WHERE price_kind = 'manual'` → 영원히 0.
- **"1박 평균"이 아니라 "1박 단가"**다. 수동값은 단가가 원본이고 총액이 파생이라
  `perNightAverage`의 규약("반드시 평균이라 불러야 한다")이 여기서만 뒤집힌다.
- **제외된 지점의 요금은 지우지 않는다.** `scheduled-refresh`의 purge 대응물을 두지 않았다 —
  남아 있어도 무동작이고, 지점을 복구하면 요금이 그대로 다시 쓰여야 한다. `/admin/rates`가
  "조회에 안 뜸" 배지로 그 상태를 보여준다.
- **고아 경고는 `destructive`가 아니다.** 지점 제외의 고아 판정은 코드 상수 `CATALOG`와
  대조해 확실하지만, 요금의 정답지는 `resort_inventory`이고 그 표는 크롤된 윈도우에만
  존재한다. 그래서 판정이 2단계이고(지점이 카탈로그에 없다 / 그 지점 재고는 있는데 이
  객실유형만 없다), **그 지점 재고가 0행이면 판정을 보류한다** — 아직 안 물어본 것과 사라진
  것은 다르다. 오탐이 섞이는 목록에 빨간색을 쓰면 진짜 오타가 그 안에서 구별되지 않는다.
- **`note`는 평문이고 `/admin/rates`에 마스킹 없이 그려진다.** `ResortAccount.memo`가 한화
  회원권 비밀번호를 담게 된 전례가 있어 스키마 주석과 다이얼로그 안내가 직접 말한다.
  보안 규칙을 늘리지는 않았다(지점 제외의 `reason`과 같은 판단).
- **소노는 뷰 변형이 접힌 행이다** — `sono/parse.ts`가 570그룹 중 300개를 접는다. 그 행에
  "그 방의 값"이 하나가 아닐 수 있으므로 `note`에 어느 변형 기준인지 적을 것. 2026-09-11부터
  행을 펼치면 변형 목록이 보이므로(아래 "소노 변형" 절) 그 목록의 라벨을 그대로 적으면 된다 —
  요금은 여전히 행당 하나다.
- ⚠️ **자동 요금이 새로 붙기 시작하면 그 지점의 수동값은 화면에서 조용히 사라진다**
  (삭제되지 않고 가려질 뿐이다). 한화 요금 조인이 뚫리면 실제로 일어난다 —
  `/admin/rates`에는 계속 보이므로 그때 정리하면 된다.

실사이트 검증(2026-09-02): 조회 전 `/api/room-rates` **0건** · 조회 후 1건(게이트 확인),
소노벨 제주 1박 170,000원 → 2박 **340,000원 · 1박 170,000원**(평균이 아니라 단가로 표기),
**롯데 속초 6행 전부 자동 요금이 있고 입력 버튼 0개**(우선순위 확인), 다이얼로그 저장이
재조회 없이 즉시 반영, 삭제 후 `＋요금` 복귀, **고아 요금(없는 객실유형)이 조회 화면에
무영향**, 매진 행에도 수동 요금 표시(게이트 면제 확인), 모바일 390px 하단 탭 5칸 정상,
콘솔 에러 0, 번들 유출 0건.

## 소노 변형 수집 — 접힌 행 아래 세부 목록 (2026-09-11 틀 · 09-13 이름·실측 완료)

운영자 질문에서 시작했다: 소노는 객실유형 아래에 세부 타입이 있다(소노벨 청송 → 리조트 스위트 →
스탠다드/파크뷰 → 취사/클린 × 더블/온돌/트윈). 그걸 수집해 보여줄 수 있는가. **답은 예이고, 운영 중이다.**
조회 결과의 소노 행을 펼치면 `스탠다드 취사/온돌 · 13실 · 예약 가능` 식으로 사이트의 예약 단위가 보인다.

**재료가 두 콜에 나뉘어 있다.** 변형(`rmTypeCd` = 뷰 × 취사 × 침대, 사이트의 예약 단위)마다의
상태·잔여는 **이미 받고 있는** `room/list/pc`에 있지만 이름이 없다(15키에 `viewCd` 코드뿐).
이름(`viewNm`·`cookNm`·`bedNm`)은 객실 선택 단계의 `room/detail`에만 있다. 그 콜은 08-31·09-07
직접 호출이 `W22M3S4 이용회원번호는 필수입니다`로 거절돼 막힌 줄 알았는데, **빠진 것은 본문의
`actualMemNo` 하나였다**(2026-09-13, 아래 "### 이름" 절).

**운영자 결정 둘**: ① 접힌 행은 그대로 두고 **행 아래에 세부 목록을 펼친다**(라벨 · 잔여 · 상태) —
행 수·유니크 키·필터 칩 건수·수동 요금 조인 키를 바꾸지 않는다(2026-09-11). ② `room/detail`이
자동으로 안 열리면 객실 선택 화면에서 이름을 읽어 표로 박는다(2026-09-13) — **열렸으므로 그 경로는
쓰지 않았다.**

- **행이 아니라 컬럼이다** — `ResortInventory.variants Json?`(jsonb, 소노만 채움). 변형을 행으로
  펼치면 소노 행이 1.5~3배가 되고 그 수가 지점·지역 칩의 "예약 가능 N건"에 그대로 들어가며,
  `(지점, roomType)`으로 조인하는 수동 요금이 전부 고아가 된다. 세부 목록은 **행의 분해이지 새
  판정이 아니다** — `removeVanishedRows`·백스톱·신선도·수동 요금 어느 것도 이 컬럼을 모른다.
  계약은 `src/lib/variants.ts`(`price.ts`와 같은 자리 — 크롤러→lib, 클라→lib).
  · `run.ts` upsert 세 곳에 추가, 행당 바인드 **16 → 17**(상한 ~3,855, 1,000 유지).
    `::jsonb` 캐스트로 바인딩하는 raw SQL의 첫 자리다. COALESCE 금지는 여기도 적용 —
    비소노 크롤러가 NULL로 덮는 것이 의도다("값의 나이 = `synced_at`").
  · `/api/inventory`는 접는 게 아니라 `isVariantList`로 **검사**한다 — 깨진 JSON은 크래시가
    아니라 "세부 목록 없음". `sw.js` `CACHE_VERSION` v4 → **v5** (2026-09-21 요금 칸이
    늘면서 **v6**).
  · **변형은 자기 요금도 들고 간다**(2026-09-21, `price: { amount, kind }`). 사이트가
    변형 하나를 묻는 콜로 답하고 같은 행의 변형끼리 값이 다르기 때문이다 — 위 "금액 수집"
    절의 소노 항목과 `AGENTS.md`의 "### 요금 수집 — 변형마다 붙는다". 요금은 **최신화
    경로에만** 붙으므로 이 칸은 대부분 비어 있고, `isVariantList`는 없는 것을 통과시킨다
    (09-21 이전에 쓰인 행이 세부 목록을 통째로 잃으면 안 된다).
- **행의 `available`이 이제 변형에서 유도된다 — 정확성 수정이 포함돼 있다.**
  `available = 어느 변형이 전 숙박 예약 가능`, `closingSoon = 가능한 변형이 전부 마감임박`.
  종전 접기는 **밤마다** 변형을 OR한 뒤 밤을 AND했다. 1박은 동치지만 2박부터 다르다 —
  1박째는 스탠다드만, 2박째는 파크뷰만 가능한 행이 초록이었는데 사이트는 `rmTypeCd` 하나로
  전 숙박을 예약하므로 어느 변형도 예약할 수 없다. 08-09에 고친 소노 2박 버그의 한 층 아래
  버전이다. **행의 존재 조건은 그대로다**(모든 밤에 엔트리가 있으면 행) — 그래서 행 수는
  변하지 않고 일부 2박 행의 `available`만 false로 내려간다. 그 수는 조사 스텝 Part 2가 잰다.
  · 부수 효과: 초록 행 아래 전부 회색인 세부 목록이라는 자기모순이 구조적으로 생기지 않는다.
- **`remaining`은 전 밤 중 최소**(`available`일 때만, 아니면 null). N박은 매 밤 방이 있어야
  하므로 가장 빠듯한 밤이 예약할 수 있는 수다. 대기(`W`)의 음수 -31은 수가 아니라 상태다.
- **이름은 사이트가 준 것만 쓴다.** `sono/names.ts`가 `room/detail`에서 받아 `rmTypeCd`로 붙이고,
  못 받은 코드는 **코드 그대로** 라벨이며 `search.ts`가 배치당 한 번 `[sono] unnamed rmTypeCd`를
  남긴다. 오크밸리 미등록 객실유형과 같은 규칙. 같은 라벨의 변형이 한 행에 둘이면 `라벨 (코드)`로
  구별한다 — 합치면 "44실 남음" 거짓이 다시 생긴다(09-13 실측 0건).
- **화면**: `RoomRow.tsx`(신규, `"use client"`)가 `<li>`와 펼침 상태를 갖고 줄 안의 내용은
  `BranchResultSection`이 그대로 `children`으로 넘긴다(상태는 셀에 — `RoomRateCell`과 같은 판단).
  변형 ≥2일 때만 쉐브론. 변형의 색은 **행의 `syncedAt`**으로 `toneOf`를 다시 부른다 — 같은
  문장으로 쓰였으니 나이가 같고, 낡은 행 아래 초록 칩은 `freshness.ts`가 막으려는 모순이다.
  어휘는 `TONE_*` 재사용(사이트의 "원활" = 우리의 "예약 가능").

### 이름 — `room/detail`은 막힌 적이 없었다 (2026-09-13)

`debug-sono.ts variants`가 SPA로 `room/detail` 200을 잡고 본문을 재생했다. **거절의 원인은 세션도
선행 콜도 헤더도 아니라 본문의 `actualMemNo`(= `memNo`) 한 칸**이었다 — 빼면 `W22M3S4`, 넣으면
저장된 쿠키만 가진 새 컨텍스트에서도 열린다. 그러니 크롤러의 storageState로 충분하다.

- **이 조사가 두 번 속을 뻔했다.** 09-07 `subtypes`는 `list/pc`의 known-good 본문을 그대로 보냈는데
  그 본문엔 애초에 `actualMemNo`가 없다. 09-13 첫 실행은 열린 응답을 `list/pc` 모양으로 세어
  말단 0 → "GO 조건 1 아니오"를 찍었다(`body[] > viewList[] > rmTypeList[]`로 한 층 더 깊다).
  **모양을 모르는 채 센 0은 '없다'가 아니다** — 오크밸리 `probe`의 "성공이 반영의 증거가 아니다"를
  뒤집은 꼴이다. 스크립트는 `detailLeavesOf`로 고쳤다.
- **세부 축 GO 조건 넷 전부 충족**: 선행 콜 0으로 `page.request`에서 열림 · 8지점 1콜 1.0초 ·
  조인 382/382 · 이름 null 0(공백 `" "` 자리표시자는 있다). 그래서 뷰 축과 세부 축을 **한 번에** 붙였다.
- 배선은 `sono/names.ts` 하나다. `search.ts`가 배치마다 `list/pc` 뒤에 부르고, `ctx.page` WeakMap으로
  **패스당 한 번**(2박 패스 0콜), 타임아웃은 `ctx.deadlineAt`에서 유도, 8초 미만이면 생략, 절대 throw
  없음. 응답에 `storeCd`가 없어 `rmTypeCd`로 키잉하고(288코드 × 32지점에서 이름 충돌 0), 칸 단위로
  병합해 비어 있지 않은 두 값이 다르면 그 코드는 이름을 포기한다.
- **라벨** = `${viewNm} ${cookNm}/${bedNm}` — 사이트 자신의 조립 규칙에서 객실유형만 뺐다.
  `rmTypeNm`("패밀리(취사/스탠다드/더블)")은 지점마다 띄어쓰기가 달라(`"골 드"`) 쓰지 않는다.
- **`SONO.viewNames`는 삭제했다.** 사람이 확인해 채울 표였는데 사이트가 답하므로 사본이 될 뿐이다.
  이름은 매 패스 `variants`에 다시 쓰이므로 사이트가 이름을 바꾸면 다음 크롤이 따라간다
  ("값의 나이 = `synced_at`" 등식 안).
- 상세·A/B 표·관측 어휘는 `AGENTS.md` 소노 절 "### 변형과 그 이름".

### 실측 (2026-09-13)

- `variants` Part 1·2(4지점 950엔트리): `viewCd` 3종(`01`·`16`·`06`), 평형 혼합 0/30,
  `rmTypeCd` → (roomTypeCd, viewCd, pyeongCd) 함수 위반 0/50, **역방향 1:1 위반 11/36**(한 뷰 안에
  취사·침대 변형이 여럿 — 뷰 이름만으로는 가를 수 없었다), 날짜·달 사이 변형 집합 변동 0,
  `rmTypeCode` 요청 필드는 무시됨, **2박 접기 오판 540행 중 2**.
- `run-crawl SONO "소노벨 청송"`: 콜드 로그인 포함 108행 15.1초, 16/16 명명.
- `CRAWL_BUDGET_MS=50000 run-crawl SONO hot`: **60/60 · 56스킵 · 20,384행 · 49.8초 1패스**.
  09-12 프로덕션 크론이 같은 스윕을 46.6초 + 21.4초 2패스로 돌았으므로 느려진 게 아니다 — 이름 콜은
  첫 윈도우의 배치당 1회(~4초)만 붙고, 넘치면 `run.ts`의 예산 게이트가 종전처럼 패스를 나눈다.
- DB: 소노 변형 **37,436개 중 코드 라벨 0**, 행↔변형 불변식(`available`·`closingSoon` 유도) 위반 0,
  펼칠 수 있는 행(변형 ≥2) 7,546, 비소노 `variants` 0.
- `tsc`·`eslint`·`next build` 통과, 번들 유출 0(`actualMemNo|room/detail|rmTypeCd|storeCd|memberReservation`).
- ⚠️ **조회 화면 육안 확인은 하지 않았다** — 이 세션에 앱 로그인 자격증명이 없다. 화면 코드
  (`RoomRow.tsx`)는 09-11 이후 무변경이고 바뀐 것은 라벨 문자열뿐이다.

오프라인 검증(2026-09-11): 합성 페이로드로 파서 불변식 확인 — 1박은 옛 접기와 판정 동치,
2박 갈림 케이스에서 행은 남고 `available`만 false, 행 수 불변, 같은 라벨 충돌 시 `(코드)` 부기,
`remaining` = 밤 최소, 변형 1개 행도 길이 1 배열.

## 마감일 계산기 (2026-08-30, 09-01 인라인 전환, 09-02 · 09-09 규칙 교체)

데스크톱 사이드바 하단의 위젯. 기준일을 고르면 **10일 전**을 알려준다(예약 마감일 역산).
달력일로 세고 휴일 보정은 **양 끝에서만** 일어난다 — 아래 "규칙" 절. `AppShell` → `DeadlineCalculator` + 지연 로드되는
`DeadlineCalculatorBody`, 계산은 `src/lib/business-days.ts`, 파싱은
`src/lib/holidays-ical.ts`, 공휴일은 `/api/holidays`(**Google 공개 캘린더 iCal 프록시,
키 불필요**).

**달력은 팝업이 아니라 사이드바 안에 상시 렌더된다**(2026-09-01, 운영자 요청). 원래
주석이 "사이드바 줄은 상시 표시판이고 팝업은 입력 장치일 뿐"이라 적고 있었는데, 이제
표시판만 남고 입력 장치가 그 안으로 들어왔다. 따라온 것 셋:

- **사이드바 폭 16→18rem**, `--app-content-w` 88→86rem(위 "디자인 시스템" 절의 한 쌍).
  달력은 `w-full`에 셀이 `flex-1 aspect-square`라 컨테이너를 그대로 따른다 —
  내용폭 272px에서 날짜 칸 38.7px로 옛 팝업(264px/37.7px)보다 오히려 넓다.
- **레일(접힘)에서는 아이콘 하나이고, 누르면 사이드바가 펼쳐진다**(`onExpand`).
  60px에 달력이 들어가지 않는다. 레일에 있던 실패 점은 사라졌는데, 접힌 상태에서는
  공휴일을 아예 받지 않아 신고할 실패가 없기 때문이다.
- **계산기 선반만 `shrink-0`이 아니다.** 달력이 ~430px를 차지하므로 버티게 두면 짧은
  뷰포트에서 계정 메뉴를 화면 밖으로 민다(`<aside>`에 스크롤 컨테이너가 없다).
  실측: 768px 높이에서 내비가 온전하고, 600px에서도 계정 메뉴가 살아남는다.

- **규칙 (2026-09-09 교체, 운영자 확인).** 두 단계이고, 오라클을 묻는 것은 둘째만이다:
  ```
  1. 카운트     기준일 − 10일 (달력일. 기준일이 주말/공휴일이어도 무관)  → rawIso
  2. 결과 보정  rawIso가 주말/공휴일이면 직전 영업일로 당긴다             → 답
  ```
  골든 케이스는 `2026-10-05(월, 쉬는 날 개천절) → 2026-09-23(수)` — 기준일이 공휴일인데
  보정 없이 −10일 = 9/25 추석 → 9/24 추석 연휴 → 9/23. 이 표가 `business-days.ts` 헤더에 있다.
  · ⚠️ **기준일은 보정하지 않는다.** 09-02 규칙은 기준일이 휴일이면 직전 영업일로 먼저
    당겼다. 운영자 요청으로 그 단계가 사라졌고 `DeadlineResult`의 `startIso`/`baseSkipped`도
    같이 지웠다. 판별 케이스 `2026-08-17(월, 쉬는 날 광복절) → 2026-08-07` — 옛 규칙이면 8/5.
  · ⚠️ **`-10`이다, `-9`가 아니다.** 09-02 규칙은 기준일을 1일째로 포함해 `-(n-1)`이었다.
    사용자 확인으로 "선택일 − 10일"로 정했다. 판별 케이스 `2026-10-16 → 2026-10-06` —
    포함이었다면 10/7.
  · **운영자 예시 둘(10/5 · 10/19)은 옛 규칙과 새 규칙이 같은 답을 낸다.** 그래서 위 판별
    케이스 둘이 `golden`에 따로 있다 — 그 둘이 없으면 교체가 검증되지 않는다.
  · **09-02 이전 규칙은 "영업일만 10개 센다"였고 틀렸다** — 같은 입력에 `10/1`·`9/17`을 냈다.
    방향이 항상 이른 쪽이라(영업일 카운트는 달력일보다 반드시 멀리 간다) 마감을 놓치게
    만들지는 않았지만, "주말 4일 · 공휴일 1일" 요약 줄이 그 틀린 답을 **설명까지 하고
    있었다.** 아래 두 검증 절의 날짜 값은 전부 그 옛 규칙의 관측이다.
- **"모른다"를 "휴일 아님"으로 읽으면 안 된다** — 그래서 판정자가 `(iso) => boolean`이
  아니라 `covers`/`isHoliday`/`nameOf`를 가진 `HolidayOracle`이다(`src/lib/holidays-kr.ts`).
  실패 방향이 나쁜 쪽이라서다: 공휴일을 적게 세면 보정이 덜 일어나 **마감일이 뒤로
  밀리고**, 사용자는 시간이 더 있다고 믿는다(규칙이 바뀌어도 이 논증은 그대로다). 판정할 수 없으면 날짜 대신 **"계산 불가"**다.
  · **이 인터페이스의 값어치는 관측됐다.** 2026-08-30에 소스를 공공데이터포털에서 Google로
    통째로 갈아끼웠는데 `business-days.ts`의 diff가 **비어 있었다**(로직도 주석도).

### 파싱이 이제 가장 위험한 코드다

- **`DESCRIPTION`이 `공휴일`/`기념일` 둘로 갈리고, 우리는 `공휴일`만 취한다**(완전 일치.
  `includes`가 아니다 — 느슨하면 표기가 바뀌었을 때 *절반만* 맞아 부분적으로 틀린 집합이
  나오고, 그게 가능한 결과 중 최악이다). 식목일·어버이날·스승의날·국군의날 등이 자동으로 빠진다.
- **⚠️ 개수 바닥이 이 기능의 새 불변식이다.** 구글이 `공휴일` 표기를 바꾸면 필터가 아무것도
  매치하지 않고 → "휴일 없는 해"처럼 보이고 → 보정이 주말에서만 일어난 **며칠 늦은 날짜**가
  나간다(연휴가 낀 구간에서 가장 크게 어긋난다).
  옛 소스에는 `resultCode === "00"`이라는 기계 성공 신호가 있었지만 여기서 성공은 **추론**이다.
  그래서 `MIN_HOLIDAYS_PER_YEAR`(10) 미만인 해는 `covered`에서 빠지고, `covered`가 비면
  라우트가 502 `label_mismatch`를 낸다. 로그에 **관측된 라벨을 그대로 찍는다** — 그게
  5분 수정과 수사(搜査)의 차이다.
- **⚠️ 그 바닥이 못 잡는 것이 하나 있다.** 구글이 **세 번째 카테고리**를 만들어 며칠만
  옮기면 나머지 20여 일이 그대로라 바닥에 안 걸리고, 그 며칠만 조용히 빠진다(= 위험한 방향).
  개수만으로는 "원래 20일인 해"와 "22일 중 2일이 딴 데로 간 해"를 구별할 수 없다.
  통제 수단은 `debug-holidays.ts diff`와 로그의 라벨 분포뿐이고 **둘 다 수동이다.**
  가드가 완전하지 않다는 것을 알고 있을 것.
- 파싱 함정 셋: **RFC 5545 줄 펴기를 분류보다 먼저**(접힘은 옥텟 기준이라 한글 한가운데서
  일어난다), **`DTEND`는 배타적**, **이스케이프 해제는 한 번의 정규식으로**(`\\`를 먼저 풀면
  `\\n`이 개행이 된다).
- **같은 날짜에 이벤트가 2건인 날이 있다**(실측 7건, 공휴일+기념일이 섞이고 순서가 일정하지
  않다). **거르고 나서 맵에 넣는다** — 넣고 나서 거르면 순서에 따라 공휴일이 기념일에 덮인다.

### 커버리지와 지평선

- **부분 성공이 두 층이다.** 문서 층 — fetch 실패·비200·`BEGIN:VCALENDAR` 아님·VEVENT 0건이면
  던진다(200에 빈 답을 주지 않는다). 연도 층 — 바닥 미달 연도는 `covered`에서 빠질 뿐이다.
  쓰레기 한 해가 나머지 열 해를 못 보게 만들지 않는다.
- **미래 상한 = 올해 + 1.** 피드는 2031년까지 주지만 먼 연도가 불완전하다 — 실측으로 2028년에
  설날 연휴 1/25이 없고 추석·개천절이 겹치는데 대체공휴일 10/5도 없다. 둘 다 휴일을 적게 세는
  = 위험한 방향이라 잘라낸다. 상한을 **서버 한 곳**에서 적용해 오라클과 달력이 같은 숫자를 본다.
- 캐시는 라우트 모듈의 단일 슬롯, TTL 12시간(무한 캐시는 임시공휴일을 영원히 못 본다),
  업스트림 실패 시 **만료 캐시라도 준다**(공휴일은 소급해 바뀌지 않는다).

### 화면에 보이는 것 둘 — 버그가 아니다

- **근로자의 날(5/1)이 휴일로 잡힌다.** Google은 넣고 특일정보(관공서 공휴일)는 뺀다.
  사기업 복지 담당자 기준으로 Google이 맞고, 방향도 안전한 쪽(더 건너뛴다 → 마감이 앞당겨진다).
  관공서 공휴일과 대조하는 사람이 버그로 읽지 않도록 여기 적어 둔다. 운영자 결정(2026-08-30).
- **`대체공휴일`이 아니라 `쉬는 날 광복절`로 보인다.** 피드 표기를 그대로 쓴다 — 정부 용어보다
  덜 정식이지만 *어느 공휴일의 대체인지*를 알려준다. 파서에서 정규화하지 않는 이유: 유지할
  변환이 하나 늘고, 위 개수 바닥이 이미 막는 표기 변경에 같이 깨지며, 정보를 버린다.

### 그 밖

- **`Calendar`의 첫 `mode="single"` 호출자다.** 프리미티브의 `selected`가 일부러 비어 있으므로
  호출부가 반드시 채운다. `onSelect`의 **2번째 인자**를 읽는다 — single 모드에서 이미 선택된
  날을 다시 누르면 1번째가 `undefined`(해제)로 와서 계산이 통째로 날아간다.
- **시각 채널 넷을 서로 다른 CSS 속성에 둔다**(RDP는 twMerge 없이 `join(" ")`한다):
  `today`=ring · `selected`=배경 · 결과=**outline** · 공휴일=`after:` 점.
  점은 **받아온 전부**에 찍는다(건너뛴 것만 찍으면 주말과 겹친 광복절에 점이 없어 "위젯이
  그 날을 모른다"처럼 읽힌다).
- **조회 화면과 일부러 다른 것 둘**: 과거를 막지 않고(`disabled` 없음), 기준일이 상태가
  아니라 **렌더 시점의 사실**이다(`picked = active ? (override ?? todayKstIso()) : null`).
  · 인라인 전환 전에는 `todayKstIso()`를 이벤트 핸들러에서만 불렀다 — 클릭이 없어졌으므로
    그 자리가 사라졌다. 하이드레이션은 `useSyncExternalStore`의 **서버 스냅샷 false**가
    막고(`AppShell`의 접힘 상태와 같은 관용구), 렌더마다 재평가되므로 자정을 넘겨 켜둔
    PWA에서 "오늘"이 밀리던 문제는 오히려 없어졌다. `useEffect`+`setState`를 쓰지 않는
    이유는 `RevealDialog.tsx:38`의 lint 오류를 하나 더 늘리지 않기 위해서다.
- `retry: 1` — 기본 3회 + 백오프는 실패를 6초 뒤에 보여줬다(실측). ⚠️ `enabled: picked !== null`을
  지우지 말 것 — 쿼리 키가 상수가 됐다고 게이트를 없애면 인증된 **모든 페이지 로드**에 요청이
  하나씩 조용히 붙는다.
  · 인라인 전환 뒤 `picked !== null`은 넷을 한꺼번에 뜻한다 — **마운트됐고 · 데스크톱이고 ·
    사이드바가 펼쳐져 있고 · 서버 렌더가 아니다.**
- ⚠️ **`hidden md:flex`는 더 이상 모바일을 막아주지 못한다.** 팝업이던 시절엔 안 보이면
  누를 수도 없어 아무 일도 없었는데, 상시 렌더가 되자 390px 뷰포트에서도 컴포넌트가
  마운트돼 `/api/holidays`를 부르고 달력 청크(~90KB)를 내려받았다(실측). 그래서
  `DESKTOP_MQ`(`min-width: 768px`)가 생겼고, 이 상수는 `<aside>`의 `md:` 분기와 **한 쌍**이다.
  어긋나면 증상이 조용하다 — 보이지도 않는 위젯이 네트워크를 쓴다. 실측 확인: 390px에서
  `/api/holidays` **0건**.
- 번들: `Calendar`가 `react-day-picker` + `ko` 로케일(~90KB)을 끌고 와, 셸에 인라인하면
  `/admin/*` 세 화면이 그걸 받는다(실측). `next/dynamic`으로 달력 본문만 잘랐다 —
  이 저장소에서 `next/dynamic`을 쓰는 첫 자리이고 근거가 그 실측이다.
  회귀 검사: 빌드 후 RDP 청크를 `client-reference-manifest.js`에서 참조하는 라우트가
  `(app)/page` 하나인지(인라인 전환 뒤에도 그대로 하나다 — 확인함).
  · **인라인 전환의 대가를 정직하게 적어 둔다**: 이제 데스크톱의 인증 페이지가 그 청크를
    실제로 **받는다**(예전엔 팝업을 연 사람만 받았다). 그래도 유지하는 이유는 둘 —
    청크가 셸의 정적 청크에 박히지 않고 하이드레이션 뒤에 오며, 모바일과 접힌 레일에서는
    `{picked && <Body/>}`가 **아예 요청하지 않는다.**
- `sw.js`는 손대지 않았다. `/api/holidays`는 SWR 목록에도 `shouldBypass`에도 없어 그대로
  네트워크로 나가고, `CACHE_VERSION` 규칙은 `/api/inventory` shape 변경에 대한 것이다.

### 이전 소스 — 공공데이터포털 특일정보 (2026-08-30에 버림)

**커밋된 적이 없으므로 이 문단이 유일한 기록이다.** 처음엔 특일정보 `getRestDeInfo`로
만들었는데, `DATA_GO_KR_SERVICE_KEY`가 끝내 발급되지 않아 기능이 한 번도 동작하지 못했다.
Google 피드는 **키가 필요 없어 그 막힘을 우회가 아니라 제거한다.** 다시 그쪽으로 갈 일이
생기면 물게 될 함정 둘: 포털이 주는 **Decoding 키**를 써야 하고(인코딩 키를 넣으면
`URLSearchParams`가 `%`를 다시 인코딩해 "키 미등록" 에러가 나는데 원인과 전혀 다르게 읽힌다),
`numOfRows` 기본값이 **10**이라 안 주면 연 20여 일 중 절반이 조용히 빠진다.
그리고 특일정보는 관공서 공휴일이라 **근로자의 날을 매년 구조적으로 빠뜨린다.**

### 검증 (2026-08-30) — **옛 규칙**

⚠️ 아래 날짜 값(8.29→8.14, 5.15→4.29, 7.31→7.16, 2026-01-05→2025-12-18)은 전부
2026-09-02에 교체된 **영업일 카운트 규칙**의 관측이다. 지우지 않는 이유는 파서·라우트·
UI 검증이 같은 절에 있고 그쪽은 여전히 유효하기 때문이다. 새 값은 아래 규칙 교체 절.

`npx tsx scripts/debug-holidays.ts golden` 16건 통과 — 걷기 6(골든 케이스 · 기준일이 공휴일 ·
월경계+설날 · 연도 넘김 · 커버리지 밖 거절 · 고장난 오라클 종료) + 파서 10(접힌 줄 · 이스케이프 ·
기념일 제외 · 배타적 `DTEND` 전개 · `VALUE=DATE-TIME` 건너뜀 · **공휴일 3건뿐인 해가 판정 불가**).
`diff` 차이 0, `feed` 라벨 분포 `{공휴일:205, 기념일:75}`.
UI(위젯을 공개 페이지에 임시로 붙여 확인 후 원복): 8.29→8.14 `쉬는 날 광복절`,
**5.15→4.29에 노동절이 실제로 건너뛰어짐**, 7.31→7.16에 제헌절 반영, 달력이 2027년 12월에서 멈춤,
2026-01-05→2025-12-18(ISO 부제목에 연도), 재클릭 멱등, 레일(42px) 팝업 288px, Escape 포커스 복귀,
(레일·팝업 항목은 2026-09-01 인라인 전환 이전의 관측이다 — 아래 재검증 참조)
타임존 3곳 하이드레이션 경고 0, API 콜 1회(캐시 공유).
**부정 경로**: 라벨을 `법정공휴일`로 바꾸면 `covered`가 빈 배열(→502), 502 두 종류 모두 화면이
**날짜가 아니라 "계산 불가"**를 그림.

### 재검증 — 인라인 전환 (2026-09-01) — **옛 규칙**

계산·파싱은 한 줄도 안 바뀌었다(`golden` 16건 그대로 통과). 바뀐 것은 배치와 게이트라
Playwright로 실제 로그인 후 확인했다:

- 1440×900에서 `<aside>` **288px**, 날짜 칸 **38.7px**, `<main>`이 x=288에서 시작.
- 골든 케이스가 **화면에서** 재현: 8/29 클릭 → `2026-08-14`, 요약 "주말 4일 · 공휴일 1일
  (쉬는 날 광복절 8.17(월))". 같은 날 재클릭 멱등(답 불변).
- `오늘` 버튼이 답과 **표시 달을 함께** 되돌린다(9월). 달력의 `month`가 인라인이라
  언마운트로 초기화되지 않으므로 `picked` 변화를 렌더 중 상태 조정으로 따라간다.
- 레일 60px → 계산기 아이콘 클릭 → 288px로 펼쳐지고 달력 복귀.
- `/api/holidays` 데스크톱 **1건** · 모바일(390px) **0건**, 콘솔 에러·하이드레이션 경고 0.
- 768px 높이에서 내비 온전(198.9 = scrollHeight), 600px에서도 계정 메뉴가 살아남음.

### 규칙 교체 1차 (2026-09-02) — **옛 규칙**

⚠️ 이 절의 3단계 규칙과 `startIso`·`기준일 … 부터` 줄은 2026-09-09에 다시 바뀌었다. 아래 2차 절.

계산 규칙이 "영업일 10개를 센다"에서 위 3단계로 바뀌었다. **파싱·라우트·캐시·커버리지·
지평선은 한 줄도 안 바뀌었다** — `holidays-ical.ts`의 diff가 비어 있고 `diff` 스텝 차이 0.
`holidays-kr.ts`도 헤더 한 줄뿐이라, 그 오라클 인터페이스가 값을 하는 **두 번째 관측**이다
(첫 번째는 2026-08-30 소스 교체).

- `golden` **18건 통과**(걷기 8 + 파서 10, 오프라인). 걷기 8은 골든 케이스 · 운영자 예시 1 ·
  기준일이 토요일+광복절 · **기준일 포함 판별** · 결과 보정이 추석 연휴 관통 · 옛 골든의
  변화 고정 · 연도 경계 · 커버리지 밖 거절, 그리고 고장난 오라클 종료.
- 라이브 피드로 운영자 예시 둘이 그대로 재현: `calc 2026-10-19` → **2026-10-08**(10일째
  10/10 토 → 한글날 10/9 → 10/8), `calc 2026-10-05` → **2026-09-23**(기준일이 대체공휴일이라
  10/2로 당겨진 뒤 −9일). `diff` 차이 0.
- 화면 문구가 규칙을 말하도록 바꿨다 — 배지 `D-10`, 화살표 줄 `10일 전 · 휴일이면 앞당김`,
  요약이 **보정이 일어난 단계만** 말한다(`기준일 10.5(월) 휴일(…) → 10.2(금)부터` /
  `10일째 10.10(토) 휴일(한글날) → 10.8(목)` / 둘 다 없으면 `10일째가 그대로 영업일입니다`).
- **`기준일이 휴일입니다` 줄을 지웠다.** 클라이언트가 `getUTCDay()`로 주말만 로컬 판정하던
  것이라 공휴일 기준일엔 뜨지 않았고, 무엇보다 **결과를 설명하지 못했다.** 정확한 조건은
  `startIso !== picked`이고 그 판정은 오라클을 거쳐서 온다.
- **달력 modifier는 늘리지 않았다.** `startIso`/`rawIso`에 표식을 더하면 시각 채널이 다섯이
  되는데, RDP가 `twMerge` 없이 `join`하므로 채널마다 서로 다른 CSS 속성이 필요하고
  (today=ring · selected=배경 · 결과=outline · 공휴일=`after:`) 남은 속성이 없다.
  설명은 텍스트 줄이 한다.
- `sw.js`·`/api/holidays`·`CACHE_VERSION` 무변경. `deadline-shared.ts`의 상수 이름만
  `LEAD_BUSINESS_DAYS` → **`LEAD_DAYS`**로 바뀌었다(더 이상 영업일이 아니다).

### 규칙 교체 2차 (2026-09-09)

운영자 요청: **기준일의 휴일 여부는 상관없다.** 선택한 날짜에서 그대로 D-10을 계산하고, 그
날이 주말/공휴일이면 그 전 영업일이 답이다. 카운트는 사용자 확인으로 **선택일 − 10일**
(기준일 비포함). 09-02의 3단계에서 1단계(기준일 보정)가 통째로 빠지고 2단계가 `-9` → `-10`.
**파싱·라우트·캐시·커버리지·지평선은 이번에도 한 줄도 안 바뀌었다** — `HolidayOracle`
인터페이스 위에서 `business-days.ts`만 바뀐 **세 번째 관측**이다.

- `business-days.ts`: `deadlineIso`가 `previousBusinessDay(baseIso)`를 부르지 않고
  `rawIso = baseIso − leadDays`에서 시작한다. `DeadlineResult` 성공 분기에서 **`startIso`·
  `baseSkipped` 삭제** — 남은 필드는 `iso` · `rawIso` · `resultSkipped`. `previousBusinessDay`는
  호출자가 하나가 됐지만 로직 무변경.
- `golden` **20건 통과**(걷기 10 + 파서 10, 오프라인). 걷기는 골든 케이스 · 운영자 예시 1 ·
  **기준일 보정 없음 판별(8/17 → 8/7)** · **기준일 비포함 판별(10/16 → 10/6)** · 결과 보정이
  추석 연휴+주말 관통 · 결과 보정이 주말만 · 기준일 토요일 · 연도 경계 · 커버리지 밖 거절 ·
  고장난 오라클 종료. 옛 `2026-08-15 → 2026-08-05` 케이스는 새 규칙에서도 8/5라 판별력이
  없어 8/17로 교체했다.
- **라이브 피드 재현은 하지 못했다.** 작업 환경에서 Google 피드가 403이었다(`curl`도 같은
  답). 코드가 아니라 네트워크 문제이고, 골든 케이스의 픽스처는 `diff`로 라이브와 일치가
  확인된 것이라 논리 검증은 그것으로 닫았다. 배포 후 `calc 2026-10-05` → 09-23,
  `calc 2026-10-16` → 10-06, `calc 2026-08-17` → 08-07을 한 번 볼 것.
- 화면: `기준일 … 휴일(…) → …부터` 줄 **삭제**(설명할 단계가 없어졌다). 남은 요약은
  `10일 전 9.25(금) 휴일(추석, 추석 연휴) → 9.23(수)` / 보정이 없으면 `10일 전이 그대로
  영업일입니다`. 배지 `D-10`과 화살표 줄 `10일 전 · 휴일이면 앞당김`은 새 규칙과 그대로
  일치해 무변경.
- `debug-holidays.ts`: `traceTable`이 2단, `describe`에서 `시작일` 조각 삭제, `calc`의
  "참고 — 단순 −10일" 줄 삭제(이제 `rawIso`와 같은 값이다).
- `sw.js`·`/api/holidays`·`CACHE_VERSION`·`LEAD_DAYS`(값 10) 무변경.
