...
`src/lib/resort-catalog.ts` + `src/components/search/{place-selection,PlaceField,PlacePicker,FilterChipRow}.tsx`.

> **2026-08-30에 표현이 전면 개편됐다** — 세 축이 패널에 나란히 펼쳐져 있던 것을
> 진입점 하나 뒤로 접었다. 아래 "장소는 진입점 하나" 항목부터 볼 것. 선택 대수
> (`place-selection.ts`)는 거의 그대로다.
...
- **장소는 진입점 하나다**(2026-08-30). 패널에 남는 장소 컨트롤은 `PlaceField` 버튼
  하나이고, 리조트·지역·지점 세 축은 전부 그 팝업(`PlacePicker`) 안에 있다. 데스크톱은
  Popover, 모바일은 바텀 Sheet이고 **본문은 같은 컴포넌트 하나**다.
  · 개편 이유는 개수가 아니라 넷이었다. ① 축 제목이 `aria-label`뿐이라 화면에 안 보였고
    두 칩 덩이가 모두 `전체`로 시작해 구분 근거가 위치밖에 없었다. ② 똑같이 생긴 칩인데
    리조트만 결과를 흐리고 재조회를 요구했다(아래 항목). ③ 축마다 레이아웃 문법이 달랐다
    (격자/wrap/시트 버튼). ④ 지점을 고르면 `selectProperty`의 백필 때문에 **누른 적 없는
    상위 칩 둘이 동시에 점등**됐다. ④는 칩이 패널에서 사라지는 것만으로 해소됐다 —
    팝업 안에서 점등되는 것은 "이 지점이 어디 소속인가"라는 빵부스러기다.
  · **규칙은 셋이다: 지역 칩 = 좁히기(즉시 · 팝업 유지) · 리조트 그룹 헤더 = 펼치기
    (선택을 안 바꾼다) · 행 = 확정(닫힘).** 목록 첫 행은 지점이 아니라 **범위 행**
    ("강원 전체")이고 지점 행과 똑같이 생겼다 — 그게 있어야 "지역만 고르고 끝내기"가
    지점 고르기와 같은 근육을 쓴다. 없으면 그 흐름의 유일한 종료 수단이 백드롭 클릭인데,
    그건 다이얼로그 관용구상 "취소"인 제스처다.
    (취소라는 개념이 애초에 없다 — 선택은 이미 적용돼 있어 **어느 경로로 닫아도 같다.**)
  · **리조트 그룹은 접혀 있다**(2026-08-30 2차). 기본 목록이 **58행 → 6행**(범위 행 +
    헤더 5)이 된다. 57지점 중 소노 32 · 한화 16으로 **두 곳이 84%**라, 접기 전에는
    목록이 `max-h-[46vh]` 창에서 6화면분이었다. 헤더에 `{n}곳`과 예약 가능 건수가 있어
    접힌 채로도 어디에 자리가 있는지 보이고, 펼치면 그 안이 다시 **지역 소제목**으로
    나뉜다(그 그룹에 지역이 2개 이상일 때만 — 소노 32곳이 강원 14 · 제주 2 …로 묶인다).
  · **리조트 칩 줄은 지웠다.** 그룹 헤더와 같은 이름 다섯 개를 두 번 말하게 돼서다.
    리조트 **선택**은 펼친 그룹 첫 행 `{리조트} 전체`가 맡는다(`browseResort` → 확정·닫힘).
  · ⚠️ **그래서 맨 위 범위 행은 리조트 축을 뺀 범위를 가리킨다** —
    `scopeLabel({resort: null, region, property: null})`. 종전처럼 `scopeLabel(value)`를
    쓰면 리조트를 고른 순간 이 행이 "한화리조트 전체"가 되어 **모든 리조트로 돌아가는 문이
    사라진다**(그 일을 하던 것이 방금 지운 칩이다). 위계는 `전체 지점 → {리조트} 전체 →
    {지점}` 세 단계다. **이 설계에서 조용히 깨질 수 있는 곳은 여기 하나다.**
  · **펼침 기본값은 파생이고 사용자 토글이 이긴다.** 검색 중 · 그룹이 하나뿐 · 선택된
    리조트나 지목된 지점을 품은 그룹 · 후보 8곳 이하면 펼침, 그 밖엔 접힘. 좁힘이 바뀌면
    기본값이 다시 적용돼야 하는데 **effect로 리셋하지 않는다**(이 저장소는 effect 안의
    setState를 lint로 막는다) — 상태에 좁힘 키를 같이 넣고 렌더에서 비교한다.
  · **검색 중에는 건수와 `{리조트} 전체` 행을 감춘다.** 건수는 현재 좁힘을 전제로 센
    패싯 값인데 검색은 일부러 그 밖을 본다(강원만 보는 중에 "제주"를 검색하면 매치가 전부
    0건으로 보인다). `{리조트} 전체`는 `{n}곳`이 매치 수인데 동작은 그 리조트 전체라
    숫자와 동작이 어긋난다.
  · **칩은 지점 고정을 푼다**(`browseResort`/`browseRegion`). `selectRegion`은 지점의
    지역이 일치하면 지점을 유지하는데, 그대로 쓰면 "설악 쏘라노 고정 + 강원 칩"이
    **아무 일도 하지 않는다.** `select*`의 백필 불변식은 건드리지 않고 합성으로 만든다 —
    `candidateProperties`·`placeLabel`·`AvailabilitySummary`의 분모가 전부 그 위에 있다.
  · **검색은 카탈로그 전체를 본다**(칩으로 좁힌 범위가 아니다). 이름을 쳐서 찾는 사람에게
    "지금 지역 칩이 제주라서 안 나옵니다"는 침묵이고, 증상이 "필터를 눌렀는데 0건"과
    구별되지 않는다. 검색 중에는 칩 줄을 감춘다 — 검색이 상위 수단이고, 둘을 같이 띄우면
    예전 패널의 난잡함을 팝업 안으로 옮기는 것에 지나지 않는다.
  · ⚠️ **`open` 상태는 갈래마다 따로 둘 것.** Popover/Sheet 본문은 포털로 나가므로
    `hidden xl:block` / `xl:hidden`이 막지 못한다. 상태를 하나로 올려 두 갈래에 내려보내면
    데스크톱에서 여는 순간 시트의 백드롭·포커스 스코프까지 마운트돼 화면이 잠긴다.
    지금 안전한 것은 **숨겨진 갈래의 트리거가 클릭될 수 없어 애초에 안 열리기** 때문이다
    (`DateRangeField`가 이미 기대고 있는 성질). 같은 이유로 오토포커스는 **데스크톱
    갈래에만** 준다 — 모바일에서 켜면 소프트 키보드가 시트를 덮는다.
- **축은 점진 노출**(`visibleAxes`). 리조트 ≥2, 지역은 **현재 선택 하의** 지역 ≥2일 때.
  · 지역 조건의 뒷항(`지점 수 > 지역 수`)은 2026-08-30에 뺐다 — 그건 **패널 세로 지면이
    비쌌기 때문에** 있던 것이고, 축이 팝업 안으로 들어가면서 근거가 사라졌다. 대신 `sel`을
    받게 해서 리조트를 골라 지역이 하나로 줄면(오크밸리는 강원뿐) 그 줄이 사라진다.
  · ⚠️ `resort` 쪽이 켜는 것은 이제 칩 줄이 아니라 **그룹 껍데기**(접기 헤더 +
    `{리조트} 전체` 행)다 — 리조트가 한 곳이면 헤더는 정보가 0이고 그 행은 범위 행과 같은
    말이라 둘 다 안 그린다.
...
- **서버 축은 날짜뿐이다**(2026-08-30, 종전 `resort` 하나). 리조트도 지역·지점과 같이
  클라이언트 `matchesPlace`가 좁히고, `matchesPlace`는 카탈로그가 아니라 **행 자신의
  `region`/`resortSlug`**로 판정한다. 그래서 `stale` 흐림은 **날짜 전용**이고,
  `Committed`에 장소 축이 하나도 없다.
  · 리조트를 뺀 이유는 성능이 아니라 **동작의 일관성**이다. 서버 축이던 시절에는
    똑같이 생긴 칩 두 줄 중 리조트만 결과를 흐리고 '조회' 재클릭을 요구했다.
    "리조트 변경 시 자동 커밋"으로 즉시화하는 길도 있었지만 그러면 `rows`가 그 리조트만
    담아 **다른 리조트 칩의 건수가 전부 0**이 된다 — 클라이언트 필터를 도입한 이유
    자체를 되돌린다.
  · 대가는 응답이 항상 전 리조트라는 것인데 **상한은 그대로다** — 기본 상태(리조트=전체)가
    이미 그랬다. **실측(2026-08-30, 운영 DB)**: 가장 큰 (checkin, checkout) 쌍이
    **449행 · 원문 188.9KB · gzip 17.6KB**(롯데 81 · 소노 169 · 오크밸리 9 · 한화 144 ·
    리솜 46). 전체 재고가 31,627행이다. 바뀌는 것은 "지점 하나 고르고 조회"하는 전형
    케이스이고, 그마저 한화 144행 → 449행이다.
  · **날짜쌍 단독 인덱스는 없다** — `where`가 `[region,checkinDate,…]`도
    `[resortId,syncedAt]`도 타지 못한다. 다만 그건 **종전 `resort` 필터도 마찬가지였다**:
    같은 윈도우를 워밍 상태에서 재 보면 전체 401~590ms, 한화만 405~537ms로 **차이가
    없다**(측정은 로컬→Neon us-east-1이라 절대값에 왕복이 섞여 있지만, 비교는 유효하다).
    즉 이 변경이 인덱스를 잃게 한 게 아니다. `@@index([checkinDate, checkoutDate])`는
    Neon 마이그레이션이라 **이번에 하지 않기로 했다.**
  · `/api/inventory`의 `resort`/`branch` 파라미터는 **남겨 둔다** — SW가 들고 있을 수 있는
    옛 캐시 URL 호환이고, 한 리조트 행수가 폭증했을 때의 탈출구다. `sw.js`의
    `CACHE_VERSION`은 올리지 않았다(응답 shape 불변, 지점 제외 작업과 같은 논거).
- **⚠️ 칩 건수는 자기 축을 빼고 센다**(패싯 카운트, `SearchView`의 `counts`).
  리조트가 서버 축이던 시절에는 `rows`가 이미 그 리조트만 담아 지역 배지가 **저절로**
  스코프됐다. 이제 `rows`가 항상 전 리조트를 담으므로 그 공짜가 사라졌고, 그냥 세면
  리조트=소노 상태에서 "강원 12"라고 떠 있는 칩을 눌렀을 때 소노 강원 3건만 나온다 —
  이 저장소가 명시적으로 거부한 실패 모드다("칩에 건수는 떠 있는데 눌러도 안 나온다").
  `place.property`는 **어느 축에도 넣지 않는다**(넣으면 지점 하나를 고른 순간 나머지
  배지가 전부 0이 되어 되돌아갈 길이 안 보인다). 배지의 계약이 "여기 뭔가 있다"에서
  **"누르면 결과가 정확히 N건"**으로 강해졌다.
- **건수 표기에 `NavBadge`를 쓰지 않는다**(`FilterChipRow`의 `FilterCount`). 그쪽 정책은
  "0이면 숨김 · 10 이상은 `9+`"인데 필터에서는 정확히 반대다 — 0이 사라지면 "여긴 없다"와
  "아직 안 세어봤다"가 같은 빈칸이 되어 **선택 불가**로 읽히고, 32와 12가 똑같은 `9+`가
  되면 지역 칩 열두 개가 전부 같은 숫자를 단다. 0건 칩은 흐리게 하되 **비활성화하지
  않는다** — 눌러서 0건을 확인하는 것이 정당한 사용이다.
...
- **지역 순서는 비교자 하나를 공유한다**(`compareRegions`). 지역이 두 곳에서 줄지어
  나오므로(지역 칩 줄 · 펼친 그룹의 소제목) 사본을 만들면 "칩은 강원→경기인데 소제목은
  가나다순"이라는 조용한 불일치가 생긴다.
- **지점 축의 표현은 하나뿐이다.** 종전에는 후보 8곳(`INLINE_MAX`)을 기준으로 칩 그리드와
  바텀시트가 갈렸는데, 개수에 따라 표현이 갈리는 것이 위 ③의 원인 중 하나였고 거의 안 도는
  쪽이 썩는다. 지금은 소규모 카탈로그(롯데 단독 4곳)도 같은 목록으로 그린다.
- **박수 스테퍼는 `DateRangeField`가 소유한다.** 예전에는 `SearchView`가 패널에 한 벌,
  날짜 시트가 또 한 벌 그렸다. 중복보다 나빴던 것은 **둘이 달랐다**는 점이다 — 시트 쪽만
  `setAwaitingCheckout(false)`를 불러서, 데스크톱에서 캘린더를 한 번 탭한 뒤 스테퍼를
  누르면 **다음 캘린더 탭이 체크인 이동이 아니라 체크아웃으로 해석**됐다.
