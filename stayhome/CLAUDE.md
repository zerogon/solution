...
## 마감일 계산기 (2026-08-30, 09-01 인라인 전환, 09-02 규칙 교체)
...
- **규칙 (2026-09-02 교체, 운영자 확인).** 세 단계이고, 첫째와 셋째가 **같은 연산**이다:
  ```
  1. 기준일 보정  기준일이 주말/공휴일이면 직전 영업일로 당긴다        → startIso
  2. 카운트       startIso를 1일째로 포함해 10일을 센다 (= -9일)      → rawIso
  3. 결과 보정    rawIso가 주말/공휴일이면 직전 영업일로 당긴다        → 답
  ```
  골든 케이스는 `2026-10-05(월, 쉬는 날 개천절) → 2026-09-23(수)` — 셋을 모두 지나가는
  유일한 케이스다. 이 표가 `business-days.ts` 헤더에 있다.
  · ⚠️ **`-9`가 오타가 아니다.** 기준일 포함이 곧 `-(n-1)`이다. `-10`으로 바꾸면
    답이 하루씩 밀리는데, **운영자가 준 예시 둘로는 그게 안 잡힌다**(우연히 같은 답이
    나온다). 그래서 `golden`에 판별 케이스 `2026-10-16 → 2026-10-07`이 따로 있다.
  · ⚠️ **보정은 카운트 *이전*이다.** 순서를 뒤집으면 10/5가 9/22가 되어 틀린다.
  · **옛 규칙은 "영업일만 10개 센다"였고 틀렸다** — 같은 입력에 `10/1`·`9/17`을 냈다.
