...
- **시각 채널 넷을 서로 다른 CSS 속성에 둔다**(RDP는 twMerge 없이 `join(" ")`한다):
  ...
  그 날을 모른다"처럼 읽힌다).
- **조회 화면과 일부러 다른 것 둘**: 과거를 막지 않고(`disabled` 없음), `todayKstIso()`를
  **렌더가 아니라 이벤트 핸들러에서만** 부른다(셸은 SSR되므로 KST↔UTC 15:00 경계에서
  하이드레이션이 갈리고, 세션 내내 사는 셸에서는 마운트 1회 메모가 자정을 넘기면 틀린다).
...
- 번들: `Calendar`가 `react-day-picker` + `ko` 로케일(~90KB)을 끌고 와, 셸에 인라인하면
  `/admin/*` 세 화면이 그걸 받는다(실측). `next/dynamic`으로 팝업 본문만 잘랐다 —
